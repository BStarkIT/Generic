...
#>
    ...
    foreach ($MB in $MBList) {
        ...
        $MBrights = Get-MailboxPermission -Identity $MB.PrimarySmtpAddress.ToString() | Where-Object {($_.User -ne "NT AUTHORITY\SELF") -and ($_.IsInherited -ne $true)} # -and ($_.AccessRights -match “FullAccess”) -and -not ($_.User -like "S-1-5*")}
    ...
    $arrPermissions | Select-Object User,'Mailbox address','Mailbox type','Access Rights'
}
