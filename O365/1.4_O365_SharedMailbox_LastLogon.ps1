$SharedMailboxUser =  Get-Mailbox -ResultSize unlimited -RecipientTypeDetails SharedMailbox | Select-Object Alias -ExpandProperty Alias
...
foreach($alias in $SharedMailboxUser)
    ...
    $Details | Export-Csv -Path C:\scripts\O365\reports\O365_SharedMailboxes_LastLogon_02032023.csv -Append -NoTypeInformation
    }
