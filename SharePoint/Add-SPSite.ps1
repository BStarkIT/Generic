...
function main() {
	...
	$ValidChoices = ($SPSite.GetWebTemplates(1033) | Select-Object Name) | ForEach-Object { $_.Name }
	$DisplayValidChoices = $SPSite.GetWebTemplates(1033) | Select-Object Name, Description
...
function Add-SPSite([string]$url,[string]$Weburl, [string]$Title, [string]$Description, [string]$Template) {
	...
	if($SPSite.AllWebs | Where-Object { $_.Title -eq $Title}) {
		...
		if($Null -ne $OpenWeb.Navigation.TopNavigationBar) {
			$TopNav = $OpenWeb.Navigation.TopNavigationBar
			[void]$TopNav.AddAsLast($Node)
		}

		if($Null -ne ($OpenWeb.Navigation.QuickLaunch | Where-Object { $_.Title -match "Sites" })) {
			$QuickLaunch = $OpenWeb.Navigation.QuickLaunch | Where-Object { $_.Title -match "Sites" }
