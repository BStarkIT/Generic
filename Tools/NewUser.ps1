...
#
# Changes       
#               V5.10   18/07/2022  BS  Ticket number added & Contractor end date method change
...
#
$SDate = Get-Date -Format "dd-MM-yyyy"
Start-Transcript -Path "\\scotcourts.local\data\CDiScripts\Scripts\Logs\New\$SDate.txt" -append
...
if ($UserName -notlike "*_a") {
    ...
    Function TribsContform {
        ...
        $Tribsform.CancelButton = $CancelButton
        $Datelabel = New-Object System.Windows.Forms.Label
        $Datelabel.Location = New-Object System.Drawing.Point(10, 385)
        $Datelabel.Size = New-Object System.Drawing.Size(280, 20)
        $Datelabel.Text = 'End date: (format 30/12/2022 )'
        $Tribsform.Controls.Add($Datelabel)
        $DateBox = New-Object System.Windows.Forms.TextBox
        $DateBox.Location = New-Object System.Drawing.Point(10, 405)
        $DateBox.Size = New-Object System.Drawing.Size(330, 20)
        $Tribsform.Controls.Add($DateBox)
        $Tribsform.Add_Shown( { $Tribsform.Activate() })    
        $dialogResult = $Tribsform.ShowDialog()
        if ($dialogResult -eq 'OK') {
            $Enddate = $DateBox.Text
    ...
    Function Judform {
        ...
        $Distributionlists = Import-Csv "\\scotcourts.local\data\CDiScripts\Scripts\Resources\Lists\JudicialUsers\distributionJud.csv"
        [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
        [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ...
    Function JudContform {
        ...
        $Datelabel.Size = New-Object System.Drawing.Size(280, 20)
        $Datelabel.Text = 'End date: (format 30/12/2022 )'
        $Judform.Controls.Add($Datelabel)
        $DateBox = New-Object System.Windows.Forms.TextBox
        $DateBox.Location = New-Object System.Drawing.Point(10, 405)
        $DateBox.Size = New-Object System.Drawing.Size(330, 20)
        $Judform.Controls.Add($DateBox)
        $Judform.Add_Shown( { $Judform.Activate() })    
        $dialogResult = $Judform.ShowDialog()
        if ($dialogResult -eq 'OK') {
            $Enddate = $DateBox.Text
    ...
    function SBuildForm {
        ...
        New-RemoteMailbox -Name "$DisplayName" -SamAccountName "$SAM" -FirstName "$FirstName" -LastName "$Lastname" -UserPrincipalName "$SAM@scotcourts.gov.uk" -OnPremisesOrganizationalUnit $OU -PrimarySmtpAddress $mail -RemoteRoutingAddress $Routingaddress -DomainController SAU-DC-04.scotcourts.local -Password $Password -ResetPasswordOnNextLogon $true
        ...
        Add-ADGroupMember -Identity "Wifi-UserCertDeployment" -Members $SAM
        ...
        Set-ADUser -Identity $SAM -Office $Office -Enabled $True -Description $Description
    ...
    function SCBuildForm {
        ...
        Add-ADGroupMember -Identity "Wifi-UserCertDeployment" -Members $SAM
        Add-ADGroupMember -Identity $SecurityGroup -Members $SAM
        if ($DistributionGroup -eq "CDi Contractors") {
            Add-ADGroupMember -Identity "CDiContractors" -Members $SAM
        }
        else {
            Add-ADGroupMember -Identity $DistributionGroup -Members $SAM 
        }
        ...
        Set-ADUser -Identity $SAM -add @{"extensionattribute2" = "USR-PERS" }
        Set-ADUser -Identity $SAM -add @{"extensionattribute3" = $Ticket }
        Set-ADUser -Identity $SAM -add @{"extensionattribute4" = "nonfte" }
        Set-ADUser -Identity $SAM -Office $Office -Enabled $True -Description $Description
        $newProxy = "smtp:" + "$SAM@scotcourtstribunals.gov.uk"
        $NewPrimary = "SMTP:" + "$SAM@scotcourts.gov.uk"
        $newProxy1 = "smtp:" + "$SAM@scotcourts.pnn.gov.uk"
    ...
    function JBuildForm {
        ...
        Set-ADUser -Identity $SAM -add @{"extensionattribute2" = "USR-PERJ" }
        Set-ADUser -Identity $SAM -add @{"extensionattribute3" = $Ticket }
    ...
    function JCBuildForm {
        ...
        $objForm.Show() | Out-Null
        Start-Sleep -Seconds 5
        Set-ADUser -Identity $SAM -ChangePasswordAtLogon $true -Office $Office -Enabled $True -Description $Description
        $newProxy = "smtp:" + "$SAM@scotcourtstribunals.gov.uk"
        $NewPrimary = "SMTP:" + "$SAM@scotcourts.gov.uk"
        $newProxy1 = "smtp:" + "$SAM@scotcourts.pnn.gov.uk"
        $newProxy2 = "smtp:" + "$SAM@scotcourtstribunals.pnn.gov.uk"
    ...
    Function Cloneform {
        $WinTitle = "New User v$version - Clone User"
        $mail = "$tentativeSAM@scotcourts.gov.uk"
        [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
        [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
        ...
        $Cloneform = New-Object System.Windows.Forms.Form
        $Cloneform.width = 800
        $Cloneform.height = 500
        $Cloneform.StartPosition = [System.Windows.Forms.FormStartPosition]::CenterScreen;
        $Cloneform.Controlbox = $false
        $Cloneform.Icon = $Icon
        $Cloneform.FormBorderStyle = 'Fixed3D'
        $Cloneform.Text = $WinTitle
        $Cloneform.Font = New-Object System.Drawing.Font('Ariel', 10)
        $CloneformBox1 = New-Object System.Windows.Forms.GroupBox
        $CloneformBox1.Location = '20,20'
        $CloneformBox1.size = '730,110'
        $CloneformBox1.text = '1. The User Will be created with the following settings:'
        $CloneformB1Text1 = New-Object System.Windows.Forms.Label
        $CloneformB1Text1.Location = '40,40'
        $CloneformB1Text1.size = '65,20'
        $CloneformB1Text1.Text = 'Name:' 
        $CloneformB1Text2 = New-Object System.Windows.Forms.Label
        $CloneformB1Text2.Location = '140,40'
        $CloneformB1Text2.size = '200,20'
        $CloneformB1Text2.ForeColor = 'Blue'
        $CloneformB1Text2.Text = $DisplayName
        $CloneformB1Text4 = New-Object System.Windows.Forms.Label
        $CloneformB1Text4.Location = '40,80'
        $CloneformB1Text4.size = '100,20'
        $CloneformB1Text4.Text = 'Email address:'
        $CloneformB1Text5 = New-Object System.Windows.Forms.Label
        $CloneformB1Text5.Location = '140,80'
        $CloneformB1Text5.size = '250,20'
        $CloneformB1Text5.ForeColor = 'Blue'
        $CloneformB1Text5.Text = $mail
        $CloneformB1Text6 = New-Object System.Windows.Forms.Label
        $CloneformB1Text6.Location = '500,40'
        $CloneformB1Text6.size = '75,20'
        $CloneformB1Text6.Text = 'Logon:'
        $CloneformB1Text7 = New-Object System.Windows.Forms.Label
        $CloneformB1Text7.Location = '575,40'
        $CloneformB1Text7.size = '100,20'
        $CloneformB1Text7.ForeColor = 'Blue'
        $CloneformB1Text7.Text = $tentativeSAM
        $CloneformB1Text8 = New-Object System.Windows.Forms.Label
        $CloneformB1Text8.Location = '500,70'
        $CloneformB1Text8.size = '75,20'
        $CloneformB1Text8.Text = 'Password:'
        $CloneformB1Text9 = New-Object System.Windows.Forms.Label
        $CloneformB1Text9.Location = '575,70'
        $CloneformB1Text9.size = '120,20'
        $CloneformB1Text9.ForeColor = 'Blue'
        $CloneformB1Text9.Text = $pass
        $CloneformBox2 = New-Object System.Windows.Forms.GroupBox
        $CloneformBox2.Location = '20,140'
        $CloneformBox2.size = '730,100'
        $CloneformBox2.text = "2. Select the User to Clone:"
        $CloneformB2Text1 = New-Object System.Windows.Forms.Label
        $CloneformB2Text1.Location = '20,35'
        $CloneformB2Text1.size = '250,30'
        $CloneformB2Text1.Text = "Existing User:" 
        $CloneformComboBox1 = New-Object System.Windows.Forms.ComboBox
        $CloneformComboBox1.Location = '325,35'
        $CloneformComboBox1.Size = '350, 310'
        $CloneformComboBox1.AutoCompleteMode = 'Suggest'
        $CloneformComboBox1.AutoCompleteSource = 'ListItems'
        $CloneformComboBox1.Sorted = $true;
        $CloneformComboBox1.DropDownStyle = [System.Windows.Forms.ComboBoxStyle]::DropDownList;
        $CloneformComboBox1.DataSource = $UsernameList
        $CloneformComboBox1.add_SelectedIndexChanged( { $Existing.Text = "$($CloneformComboBox1.SelectedItem.ToString())" })
        $Existing = New-Object System.Windows.Forms.Label
        $Existing.Location = '20,70'
        $Existing.Size = '200,20'
        $Existing.ForeColor = 'Blue'
        $OKButton = New-Object System.Windows.Forms.Button
        $OKButton.Location = '525,400'
        $OKButton.Size = '100,40'          
        $OKButton.Text = 'Ok'
        ...
        $CancelButton.add_Click( {
                ...
                $Cloneform.Dispose()
                Return MainForm })
        $CloneformBox1.Controls.AddRange(@($CloneformB1Text1, $CloneformB1Text2, $CloneformB1Text4, $CloneformB1Text5, $CloneformB1Text6, $CloneformB1Text7, $CloneformB1Text8, $CloneformB1Text9))
        $CloneformBox2.Controls.AddRange(@($CloneformB2Text1, $CloneformComboBox1, $Existing))
        $Cloneform.Controls.AddRange(@($CloneformBox1, $CloneformBox2, $OKButton, $CancelButton))
        $Cloneform.AcceptButton = $OKButton
        $Cloneform.CancelButton = $CancelButton
        $Cloneform.Add_Shown( { $Cloneform.Activate() })    
        $dialogResult = $Cloneform.ShowDialog()
        if ($dialogResult -eq 'OK') {
            if ($CloneformComboBox1.Text -eq '') {
                Write-Host "no clone user selected"
                [System.Windows.Forms.MessageBox]::Show("You need to select a Username !!!!!  Trying to enter blank fields is never a good idea.", 'Clone an existing user.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
                $Cloneform.Close()
                $Cloneform.Dispose()
                break
            }
                ...
                $MailboxCheck = Get-ADUser -filter { DisplayName -eq $CloneformComboBox1.Text } -Properties * | Select-Object EmailAddress | Select-Object -ExpandProperty EmailAddress
                If ($null -eq $MailboxCheck) {
                    [System.Windows.Forms.MessageBox]::Show("The selected User does not have a mailbox`n`nProcessing cannot continue.", 'Clone an existing user.',
                ...
                elseif ($Type = "USR-PERT") {
                    ...
                    [System.Windows.Forms.MessageBox]::Show("The selected User is not suitable for cloning`n`nProcessing cannot continue.", 'Clone an existing user.',
                        [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                    $Cloneform.Close()
                    $Cloneform.Dispose()
                    Write-Host "No Attributes back to main form." 
                    Return MainForm 
                }
    ...
    Function CloneCform {
        ...
        $UserNameList = Get-ADUser -filter * -searchbase 'ou=soe users 2.6,ou=scts users,ou=user accounts,ou=scts,DC=scotcourts,DC=local' -Properties DisplayName | Select-Object Displayname | Select-Object -ExpandProperty DisplayName
        Write-Output "Pulling User Accounts"
        $CloneCform = New-Object System.Windows.Forms.Form
        $CloneCform.width = 800
        $CloneCform.height = 500
        $CloneCform.StartPosition = [System.Windows.Forms.FormStartPosition]::CenterScreen;
        $CloneCform.Controlbox = $false
        $CloneCform.Icon = $Icon
        $CloneCform.FormBorderStyle = 'Fixed3D'
        $CloneCform.Text = $WinTitle
        $CloneCform.Font = New-Object System.Drawing.Font('Ariel', 10)
        $CloneCformBox1 = New-Object System.Windows.Forms.GroupBox
        $CloneCformBox1.Location = '20,20'
        $CloneCformBox1.size = '730,110'
        $CloneCformBox1.text = '1. The User Will be created with the following settings:'
        $CloneCformB1Text1 = New-Object System.Windows.Forms.Label
        $CloneCformB1Text1.Location = '40,40'
        $CloneCformB1Text1.size = '65,20'
        $CloneCformB1Text1.Text = 'Name:' 
        $CloneCformB1Text2 = New-Object System.Windows.Forms.Label
        $CloneCformB1Text2.Location = '140,40'
        $CloneCformB1Text2.size = '200,20'
        $CloneCformB1Text2.ForeColor = 'Blue'
        $CloneCformB1Text2.Text = $DisplayName
        $CloneCformB1Text4 = New-Object System.Windows.Forms.Label
        $CloneCformB1Text4.Location = '40,80'
        $CloneCformB1Text4.size = '100,20'
        $CloneCformB1Text4.Text = 'Email address:'
        $CloneCformB1Text5 = New-Object System.Windows.Forms.Label
        $CloneCformB1Text5.Location = '140,80'
        $CloneCformB1Text5.size = '250,20'
        $CloneCformB1Text5.ForeColor = 'Blue'
        $CloneCformB1Text5.Text = $mail
        $CloneCformB1Text6 = New-Object System.Windows.Forms.Label
        $CloneCformB1Text6.Location = '500,40'
        $CloneCformB1Text6.size = '75,20'
        $CloneCformB1Text6.Text = 'Logon:'
        $CloneCformB1Text7 = New-Object System.Windows.Forms.Label
        $CloneCformB1Text7.Location = '575,40'
        $CloneCformB1Text7.size = '100,20'
        $CloneCformB1Text7.ForeColor = 'Blue'
        $CloneCformB1Text7.Text = $tentativeSAM
        $CloneCformB1Text8 = New-Object System.Windows.Forms.Label
        $CloneCformB1Text8.Location = '500,70'
        $CloneCformB1Text8.size = '75,20'
        $CloneCformB1Text8.Text = 'Password:'
        $CloneCformB1Text9 = New-Object System.Windows.Forms.Label
        $CloneCformB1Text9.Location = '575,70'
        $CloneCformB1Text9.size = '120,20'
        $CloneCformB1Text9.ForeColor = 'Blue'
        $CloneCformB1Text9.Text = $pass
        $CloneCformBox2 = New-Object System.Windows.Forms.GroupBox
        $CloneCformBox2.Location = '20,140'
        $CloneCformBox2.size = '730,100'
        $CloneCformBox2.text = "2. Select the User to Clone:"
        $CloneCformB2Text1 = New-Object System.Windows.Forms.Label
        $CloneCformB2Text1.Location = '20,35'
        $CloneCformB2Text1.size = '250,30'
        $CloneCformB2Text1.Text = "Existing User:" 
        $CloneCformComboBox1 = New-Object System.Windows.Forms.ComboBox
        $CloneCformComboBox1.Location = '325,35'
        $CloneCformComboBox1.Size = '350, 310'
        $CloneCformComboBox1.AutoCompleteMode = 'Suggest'
        $CloneCformComboBox1.AutoCompleteSource = 'ListItems'
        $CloneCformComboBox1.Sorted = $true;
        $CloneCformComboBox1.DropDownStyle = [System.Windows.Forms.ComboBoxStyle]::DropDownList;
        $CloneCformComboBox1.DataSource = $UsernameList
        $CloneCformComboBox1.add_SelectedIndexChanged( { $Existing.Text = "$($CloneCformComboBox1.SelectedItem.ToString())" })
        $Existing = New-Object System.Windows.Forms.Label
        $Existing.Location = '20,70'
        $Existing.Size = '200,20'
        $Existing.ForeColor = 'Blue'
        $Datelabel = New-Object System.Windows.Forms.Label
        $Datelabel.Location = New-Object System.Drawing.Point(10, 255)
        $Datelabel.Size = New-Object System.Drawing.Size(280, 20)
        $Datelabel.Text = 'End date: (format 30/12/2022 )'
        $CloneCform.Controls.Add($Datelabel)
        $DateBox = New-Object System.Windows.Forms.TextBox
        $DateBox.Location = New-Object System.Drawing.Point(10, 285)
        $DateBox.Size = New-Object System.Drawing.Size(330, 20)
        $CloneCform.Controls.Add($DateBox)
        $OKButton = New-Object System.Windows.Forms.Button
        $OKButton.Location = '525,400'
        $OKButton.Size = '100,40'          
        $OKButton.Text = 'Ok'
        ...
        $CancelButton.add_Click( {
                ...
                $CloneCform.Dispose()
                Return MainForm })
        $CloneCformBox1.Controls.AddRange(@($CloneCformB1Text1, $CloneCformB1Text2, $CloneCformB1Text4, $CloneCformB1Text5, $CloneCformB1Text6, $CloneCformB1Text7, $CloneCformB1Text8, $CloneCformB1Text9))
        $CloneCformBox2.Controls.AddRange(@($CloneCformB2Text1, $CloneCformComboBox1, $Existing))
        $CloneCform.Controls.AddRange(@($CloneCformBox1, $CloneCformBox2, $OKButton, $CancelButton))
        $CloneCform.AcceptButton = $OKButton
        $CloneCform.CancelButton = $CancelButton
        $CloneCform.Add_Shown( { $CloneCform.Activate() })    
        $dialogResult = $CloneCform.ShowDialog()
        if ($dialogResult -eq 'OK') {
            $Enddate = $DateBox.Text
            if ($DateBox.Text -eq '') {
                Write-Host "No Enddate entered"
            ...
            if ($CloneCformComboBox1.Text -eq '') {
                Write-Host "no clone user selected"
                [System.Windows.Forms.MessageBox]::Show("You need to select a Username !!!!!  Trying to enter blank fields is never a good idea.", 'Clone an existing user.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
                $CloneCform.Close()
                $CloneCform.Dispose()
                ...
                # CHECK -  User has email address   #
                $MailboxCheck = Get-ADUser -filter { DisplayName -eq $CloneCformComboBox1.Text } -Properties * | Select-Object EmailAddress | Select-Object -ExpandProperty EmailAddress
                If ($null -eq $MailboxCheck) {
                    [System.Windows.Forms.MessageBox]::Show("The selected User does not have a mailbox`n`nProcessing cannot continue.", 'Clone an existing user.',
                        [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                    $CloneCform.Close()
                ...
                if (($Tester -in $Safe.OfficeList) -or ($Tester -in $Safe.OfficeTribs) -or ($Tester -in $Safe.officeJud)) {
                    ...
                    Write-host "Selected user is Non-standard"
                    Return MainForm
                }
            }
        }
    ...
    Function MainForm {
        ...
        [int]$inc = 0
        [int]$ind = 0
        $JudFirstName = Import-Csv "\\scotcourts.local\data\CDiScripts\Scripts\Resources\Lists\JudicialUsers\FirstNameJud.csv"
        $ManForm = New-Object System.Windows.Forms.Form
        $ManForm.Icon = $Icon
        $ManForm.Size = New-Object System.Drawing.Size(375, 500)
        ...
        $OKButton = New-Object System.Windows.Forms.Button
        $OKButton.Location = New-Object System.Drawing.Point(100, 425)
        ...
        $CancelButton = New-Object System.Windows.Forms.Button
        $CancelButton.Location = New-Object System.Drawing.Point(175, 425)
        ...
        $BoxRadioButton1.Location = '20,300'
        $BoxRadioButton1.size = '200,30'
        $BoxRadioButton1.Checked = $true 
        $BoxRadioButton1.Text = 'SCTS.'
        $BoxRadioButton2 = New-Object System.Windows.Forms.RadioButton
        $BoxRadioButton2.Location = '20,330'
        $BoxRadioButton2.size = '200,30'
        $BoxRadioButton2.Checked = $false
        $BoxRadioButton2.Text = 'Tribs.'
        $BoxRadioButton3 = New-Object System.Windows.Forms.RadioButton
        $BoxRadioButton3.Location = '20,360'
        $BoxRadioButton3.size = '100,30'
        $BoxRadioButton3.Checked = $false
        $BoxRadioButton3.Text = 'Judicial.'
        $BoxRadioButton4 = New-Object System.Windows.Forms.RadioButton
        $BoxRadioButton4.Location = '20,390'
        $BoxRadioButton4.size = '100,30'
        $BoxRadioButton4.Checked = $false
        $BoxRadioButton4.Text = 'Clone.'
        $BoxRadioButton4.Add_Click( {
        ...
        $ManForm.Controls.Add($BoxRadioButton4)
        ...
        $tempfirstname = $textBox1.Text
        $Templastname = $textBox2.Text
        $Title = $JudcomboBox.Text
        $TicketTrace = $textBox3.Text
        if ($Result -eq 'OK') {
            $charlist1 = [char]97..[char]122 # Lower
            $charlist2 = [char]65..[char]90 # Upper
            $charlist3 = [char]48..[char]57 # Number
            $charlist4 = [char]33..[char]38 + [char]40..[char]43 + [char]45..[char]46 + [char]64 # symbols
            $pwdList = @()
            $pwLength = 1 
            ...
            $TicketTraceOK = $TicketTrace -replace '[^\x30-\x39]+', ''
            if ($textBox3.Text -eq '') {
            ...
            Else {
                $Ticket = "Created on Ticket : $TicketTraceOK"
                if ($ContBox.Checked) {
                    Write-host "Contractor"
                }
                if ($BoxRadioButton1.Checked) {
                    ...
                    if (Get-ADUser -Filter { displayName -eq $TempDisplayName }) {    
                        do {
                            $ind ++
                            $TempDisplayName = $DisplayName + [string]$ind
                        } 
                ...
                elseif ($BoxRadioButton4.Checked) {
                    $TempDisplayName = $LastName + ", " + $FirstName
                    $tentativeSAM = ($firstname.substring(0, 1) + $lastname).toLower()
                    $samcatch = $tentativeSAM
                    $DisplayName = $TempDisplayName
                ...
                elseif ($BoxRadioButton3.Checked) {
