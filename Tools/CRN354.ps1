...
$LeaversList = Get-ADUser -Filter * -SearchBase 'OU=CRN354-22,OU=Z-Disabled_Leavers,OU=User Accounts,OU=SCTS,DC=scotcourts,DC=local' -Properties Name, description, DistinguishedName | Select-Object Name, SamAccountName, Description, DistinguishedName | Where-Object { $_.Description -match "xx DELETE after DATE --" }
# Script begin
...
Foreach ($expiredAccount in $expiredAccounts) {
    ...
    if ($Accountcheck.extensionattribute4 -eq "nonfte") {
        ...
        if ($Accountcheck.AccountExpirationDate -lt $1monthgrace) {
            ...
            $pdrive = test-path \\scotcourts.local\home\P\$Excontractor
            try {
                If (($null -ne $Usermembership) -and ($pdrive -eq $true)) {                   
                    $Usermembership | out-file \\scotcourts.local\home\P\$Excontractor\UserMembershipBackup.csv
            ...
            try {
                if (test-path \\scotcourts.local\Home\P\$Excontractor) {
                    Get-Item \\scotcourts.local\home\P\$Excontractor | Rename-Item -NewName { $_.Name -replace "$", " xx DELETE after DATE - $Datedel" }
                    Write-Host "Renamed $Excontractor P drive folder on SAUFS01" 
                }
            }
            catch {
                Write-Host "Error - Cannot Renamed $Excontractor P drive folder on SAUFS01" 
            }
            Set-ADUser -Identity $Excontractor -Replace @{msExchHideFromAddressLists = $true }
            Set-ADUser $Accountcheck.SamAccountName -Description "$($_.Description) xx DELETE after DATE -- $Datedel"
            $GUID = Get-aduser -Identity $Excontractor -Properties * | Select-Object -ExpandProperty ObjectGUID
            Move-ADObject -Identity $GUID -targetpath 'OU=Z-Disabled_Leavers,OU=User Accounts,OU=SCTS,DC=scotcourts,DC=local'
        }
    }
}
if ($null -eq $LeaversOlderThan1MonthList) {
    ...
    try {
        ForEach ($User in $LeaversOlderThan1MonthList.SamAccountName) {
            Write-Host $User
            $delete = get-aduser -Identity $user | Select-Object -ExpandProperty DistinguishedName
            Remove-ADObject -Identity $delete -Recursive -Confirm:$False
        }
    }
    catch {
