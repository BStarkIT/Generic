...
#           
# Issues:       Line 1118 Sharedmailbox not yet removed automatically               
#               Line 3761 Distrubutiuon list errors
#
# Revisions     v1.5 30/03/2020 BS - updated to office 2016, added Win10 user groups, SCTS Distribution lists & revised syntax
...
Function AddMBFullAccessPermissionForm {
    ...
    if ($dialogResult -eq 'OK') {
        ...
        Else {
            ...
            #####################################################################
            Add-Type -AssemblyName System.Windows.Forms 
            [System.Windows.Forms.MessageBox]::Show("The user ( $($AddMBFullUserNameComboBox1.SelectedItem.ToString()) ) has had Full Access Permissions added to the ( $($AddMBFullMBNameComboBox2.SelectedItem.ToString()) ) mailbox.  The user needs to close and re-open Outlook and  the mailbox will appear in a few minutes.", 'Mailbox - Add Full Access Permissions for a User', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
            #####################################################################
...
Function SharedLogOnForm {
    ...
    $StartMessage = [System.Windows.Forms.MessageBox]::Show("This script manages Shared Mailboxes.  Note 1: When prompted for a username & password enter:  scs\YourLogOnName & password  Note 2:When prompted for an Outlook profile add a new profile named:  SharedMailbox  Note 3:When adding new profile the email address needs changed to Shared Mailbox Name  Please click OK to continue or Cancel to exit", 'Shared Mailbox - Manage Shared Mailboxes.', [System.Windows.Forms.MessageBoxButtons]::OKCancel, [System.Windows.Forms.MessageBoxIcon]::Warning)
    if ($StartMessage -eq 'Cancel') { exit } 
    ### Check if Outlook running  ###
    if ((get-process "outlook" -ea SilentlyContinue) -ne $Null) {
        Add-Type -AssemblyName System.Windows.Forms 
        [System.Windows.Forms.MessageBox]::Show("Outlook is currently running!!!!.  You need to close Outlook before running this script.", 'Shared Mailbox - Manage Shared Mailboxes', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
        Return SharedMailboxManagementForm
    }
    else {
        [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
        ...
        if ($dialogResult -eq 'OK') {
            ...
            Else {
                ...
                Add-MailboxPermission $MailBoxName -User $UserName -AccessRights FullAccess -confirm:$false -InheritanceType All -Automapping $false
                ...
                If ((Test-Path -Path "Registry::\HKEY_USERS\$SID\Software\Microsoft\Windows NT\CurrentVersion\Windows Messaging Subsystem\Profiles\SharedMailbox") -ne '$true') {
                    ...
                    if (($UserName | Measure-Object).count -ne 1) { SharedLogOn }
                    if (($MailBoxName | Measure-Object).count -ne 1) { SharedLogOn }
                    Remove-MailboxPermission $MailBoxName -User $UserName -AccessRights FullAccess -confirm:$false
                    $SharedLogOnForm.Close()
                    $SharedLogOnForm.Dispose()
                    Return SharedMailboxManagementForm
                }
...
Function MainForm {
    ...
    $Ex2013ManForm.Text = 'Exchange 2013 Management Main Form v1.5'
