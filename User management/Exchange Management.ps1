...
# Revisions     
#               V2.11 18/08/2021 BS - distribution OU moved to Legacy
#               V2.10 17/08/2021 BS - RBAC considerations added
#               V2.00 25/08/2020 BS - Bring in line with other management scripts
#               V1.57 22/08/2020 BS - Cleanup of code.
#               V1.56 15/07/2020 BS - Corrected error on email to helpdeks lines.
...
Start-Transcript -Path "\\scotcourts.local\data\IT\Enterprise Team\UserManagement\Logging\Exchange\$Date.txt" -append
...
$Version = '3.01'
$WinTitle = "HelpDesk control script v$version."
########################################################################################################
################     Get Shared mailboxes & Distribution Lists from AD    ##############################
########################################################################################################
$Sharedmailboxes = Get-Mailbox -OrganizationalUnit 'OU=Shared Mailboxes,OU=Resource Accounts,OU=SCTS,DC=scotcourts,DC=local' | Select-Object DisplayName | Select-Object -ExpandProperty DisplayName 
Write-Output "Pulling Shared mailboxes"
$Distributionlists = Get-DistributionGroup -OrganizationalUnit 'OU=Distribution Lists,OU=Groups,OU=SCTS,DC=scotcourts,DC=local' | Select-Object Name | Select-Object -ExpandProperty Name  
...
Function AddMBFullAccessPermissionForm {
    $WinTitle = "Mailbox - Add Full Access Permissions for a User v$version."
    ...
    $CancelButton.add_Click( {
            Write-output "Return to previous forms"
            $AddMBFullForm.Close()
    ...
    if ($dialogResult -eq 'OK') {
        ...
        Elseif ($AddMBFullSelectedMailBoxNametextLabel6.Text -eq '') {
            Write-Output "no mailbox selected"
        ...
        If ($Status.AccessRights -eq 'FullAccess') {
            Write-Output "User $UserSamAccountName already has full access to mailbox $MailBoxPrimarySMTPAddress"
        ...
        Else {
            ...
            Write-Output "User $UserSamAccountName has been added to mailbox $MailBoxPrimarySMTPAddress"
            #####################################################################
...
Function AddMBSendBehalfOfAccessPermissionForm {
    $WinTitle = "Mailbox - Add Send On Behalf Of Access Permissions for a User v$version."
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $AddMBSendBehalfOfForm.Text = $WinTitle
    ...
    $CancelButton.Text = 'Cancel back to Form'
    $CancelButton.add_Click( {
            Write-Output "return to previous form"
    ...
    if ($dialogResult -eq 'OK') {
        ...
        If ($Status -match "$UserDisplayName.*") {
            Write-Output "the user $UserSamAccountName already has send on behalf permisson on mailbox $MailBoxPrimarySMTPAddress"
            Add-Type -AssemblyName System.Windows.Forms 
        ...
        Else {
            ...
            Write-Output "the user $UserSamAccountName has been given send on behalf permisson on mailbox $MailBoxPrimarySMTPAddress"
            #####################################################################
            ##################  Message complete message  #######################
            #####################################################################
            Add-Type -AssemblyName System.Windows.Forms 
...
Function CheckMBFullAccessPermissionForm {
    $WinTitle =  "Mailbox - Check Full Access Permissions on Mailbox v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    if ($dialogResult -eq 'OK') {
        ...
        Write-Output "mailbox $MailboxName permissions shown"
...
Function CheckMBSendOnBehalfPermissionForm {
    $WinTitle =  "Mailbox - Check Send On Behalf Of permissions on Mailbox v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $CheckMBSendOnBehalfForm.Text = $WinTitle
    ...
    if ($dialogResult -eq 'OK') {
        ...
        Write-Output "List of Users with Send On Behalf Of permissions on $MailboxName mailbox"
...
Function RemoveMBFullAccessPermissionForm {
    $WinTitle =  "Mailbox - Remove Full Access Permissions for a User v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $RemoveMBFullForm.Text = $WinTitle
    ...
    $CancelButton.Text = 'Cancel back to Form'
    $CancelButton.Add_Click( {
            Write-Output "Return to main form"
    ...
    if ($dialogResult -eq 'OK') {
        ...
        If ($Status.AccessRights -ne 'FullAccess') {
            Write-Output "User $UserSamAccountName doesn't have full access to $MailBoxPrimarySMTPAddress"
            Add-Type -AssemblyName System.Windows.Forms 
        ...
        Else {
            ...
            Write-Output "User $UserSamAccountName access to $MailBoxPrimarySMTPAddress removed"
            #####################################################################
            ##################  Message complete message  #######################
            #####################################################################
            Add-Type -AssemblyName System.Windows.Forms 
...
Function RemoveMBSendOnBehalfPermissionForm {
    $WinTitle =  "Mailbox - Remove Send On Behalf Of permissions for a User v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $RemoveMBSendOnBehalfForm.Text = $WinTitle
    ...
    if ($dialogResult -eq 'OK') {
        ...
        If ($Status1 -eq "") {
            Add-Type -AssemblyName System.Windows.Forms 
            write-output "User $UserSamAccountName doesnt have sent on behalf on $MailBoxPrimarySMTPAddress"
        ...
        Else {
            ...
            #####################################################################
            Add-Type -AssemblyName System.Windows.Forms 
            Write-Output "User $UserSamAccountName removed from sent on behalf on $MailBoxPrimarySMTPAddress"
...
Function SharedLogOnForm {
    ...
    if ($Null -ne (Get-Process "outlook" -ea SilentlyContinue)) {
        Write-Output "Outlook still running"
        Add-Type -AssemblyName System.Windows.Forms 
        ...
        $WinTitle =  "Shared Mailbox - Manage Shared Mailboxes v$Version"
        [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
        [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
        ### Set the details of the form. ###
        ...
        $CancelButton.Text = 'Cancel back to Form'
        $CancelButton.Add_Click( {
                write-output "return to mail form"
        ...
        if ($dialogResult -eq 'OK') {
            ...
            Else {
                ...
                $UserSamName = $UserName.Replace("_a","")
                ...
                Write-Output "the User $UserSamName has been given temp access to $MailBoxName"
                ...
                Else {
                    ...
                    Write-Output "the User $UserSamName removed from $MailBoxName"
                    If ((Test-Path -Path "Registry::\HKEY_USERS\$SID\Software\Microsoft\Office\16.0\Outlook\Profiles\SharedMailbox") -eq '$true') {
                        Remove-Item -Path "Registry::\HKEY_USERS\$SID\Software\Microsoft\Office\16.0\Outlook\Profiles\SharedMailbox" -Recurse
                    }
                    $SharedLogOnForm.Close()
...
Function AddNewSharedMBForm {
    ...
    if ($StartMessage -eq 'Cancel') { exit } 
        ...
        $WinTitle =  "Add New Shared Mailbox v$Version"
        [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
        [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
        ### Set the details of the form. ###
        ...
        if ($dialogResult -eq 'OK') {
            ...
            if ($SharedMBNewtextBox1.text -eq '') {
                Write-Output "no details entered"
            ...
            If ($Null -ne $ListEmailAddress) {
                Write-Output "the address $EmailName is already in use"
                Add-Type -AssemblyName System.Windows.Forms 
            ...
            Else {
                ...
                #####################################################################
                New-Mailbox -shared  -Name "$DisplayName" -Alias $EmailName -UserprincipalName "$EmailName@scotcourts.local" -OrganizationalUnit 'OU=Shared Mailboxes,OU=Resource Accounts,OU=SCTS,DC=scotcourts,DC=local'
                # 
                ##########################################################################
                ...
                function changeit {  
                    $script:Sharedmailboxes = Get-Mailbox -OrganizationalUnit 'OU=Shared Mailboxes,OU=Resource Accounts,OU=SCTS,DC=scotcourts,DC=local' | Select-Object DisplayName | Select-Object -ExpandProperty DisplayName 
                }  
                changeit 
                ##########################################################################
                ...
                Write-Output "Shared mailbox $EmailName created"
                $SharedMBNewForm.Close()
                $SharedMBNewForm.Dispose()
                Return SharedMailboxManagementForm
            }
...
Function AddNewTribunalSharedMBForm {
    ...
    if ($StartMessage -eq 'Cancel') { exit } 
        ...
        $WinTitle =  "Add New Tribunal Shared mailbox v$Version"
        [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
        [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
        ### Set the details of the form. ###
        ...
        $TribSharedMBNewForm.Text = $WinTitle
        ...
        if ($dialogResult -eq 'OK') {
            ...
            If ($Null -ne $ListEmailAddress) {
                Add-Type -AssemblyName System.Windows.Forms
                Write-Output "the address $EmailName is in use" 
            ...
            Else {
                ...
                #######            Add description & mailbox  owner    ###################
                ##########################################################################
                write-output "new tribs mailbox $DisplayName - $EmailName created"
...
Function CopySharedMBForm {
    $WinTitle =  "Add copy of sent mail to shared mailbox sent folder form v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    if ($dialogResult -eq 'OK') {
        ...
        Else {
            ...
            Write-Output "Copy send as enabeld on $MailBoxPrimarySMTPAddress"
...
Function OutOfficeCheckForm {
    $WinTitle =  "User Mailbox - Out Of Office - Check current status v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $OutOfficeCheckForm.Text = $WinTitle
    ...
    if ($dialogResult -eq 'OK') {
        ...
        If ($Status.AutoReplyState -eq 'Disabled') {
            Write-Output "OOO is Off for $UserSamAccountName"
            Add-Type -AssemblyName System.Windows.Forms 
        ...
        ElseIf ($Status.AutoReplyState -eq 'Enabled') {
            Write-Output "OOO is On for $UserSamAccountName"
            Add-Type -AssemblyName System.Windows.Forms 
...
Function OutOfficeTurnOnForm {
    $WinTitle =  "User Mailbox - Out Of Office - Turn On v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $OutOfficeTurnOnForm.Text = $WinTitle
    ...
    if ($dialogResult -eq 'OK') {
        ...
        If ($Status.AutoReplyState -eq 'Enabled') {
            Write-Output "$UserSamAccountName OOO already on"
            Add-Type -AssemblyName System.Windows.Forms 
        ...
        Else {
            ...
            Write-Output "$UserSamAccountName OOO turned on"
            Add-Type -AssemblyName System.Windows.Forms 
...
Function OutOfficeTurnOffForm {
    $WinTitle =  "User Mailbox - Out Of Office - Turn Off v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $OutOfficeTurnOffForm.Text = $WinTitle
    ...
    if ($dialogResult -eq 'OK') {
        ...
        $Status = Get-MailboxAutoReplyConfiguration $UserSamAccountName | Select-Object AutoReplyState  
        If ($Status.AutoReplyState -eq 'Disabled') {
            Write-Output "$UserSamAccountName OOO already off"
            Add-Type -AssemblyName System.Windows.Forms 
        ...
        Else {
            ...
            Write-Output "$UserSamAccountName OOO turned off"
            Add-Type -AssemblyName System.Windows.Forms 
...
Function OutOfficeAddTurnOnForm {
    $WinTitle =  "User Mailbox - Out Of Office - Add message and turn On v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $OutOfficeAddUserForm.Text = $WinTitle
    ...
    if ($dialogResult -eq 'OK') {
        ...
        Elseif ($OutOfficeMessagetextBox2.Text -eq '') {
            Write-Output "no message entered"
        ...
        ##################  Message complete message  #######################
        #####################################################################
        Write-Output "$UserSamAccountName OOO updated"
        Add-Type -AssemblyName System.Windows.Forms 
...
Function OutOfficeCheckMessageForm {
    $WinTitle =  "User Mailbox - Out Of Office - Check Message v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $OutOfficeCheckMessageForm.Text = $WinTitle
    ...
    if ($dialogResult -eq 'OK') {
        ...
        Write-Output "$UserSamAccountName OOO displayed"
...
Function AddCalOwnerPermissionForm {
    $WinTitle =  "Calendar - Add Owner (Full Access) Permissions for a User v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    if ($dialogResult -eq 'OK') {
        ...
        Elseif ($AddCalOwnerSelectedMailBoxNametextLabel6.Text -eq '') {
            Write-output "No Calendar selected"
        ...
        If ($Status.AccessRights -eq 'Owner') {
            Write-Output "user $UserSamAccountName already has $MailBoxPrimarySMTPAddress calendar access as Owner"
            Add-Type -AssemblyName System.Windows.Forms 
        ...
        If ($Status.AccessRights -eq 'Reviewer') {
            Write-Output "user $UserSamAccountName already has $MailBoxPrimarySMTPAddress calendar access as Reviewer"
        ...
        Else {
            ...
            Write-Output "user $UserSamAccountName added to $MailBoxPrimarySMTPAddress calendar as Owner"
            #####################################################################
            ##################  Message complete message  #######################
            #####################################################################
            Add-Type -AssemblyName System.Windows.Forms 
...
Function AddCalReviewerPermissionForm {
    $WinTitle =  "Calendar - Add Reviewer Permissions for a User v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $AddCalReviewerForm.Text = $WinTitle
    ...
    if ($dialogResult -eq 'OK') {
        ...
        If ($Status.AccessRights -eq 'Reviewer') {
            Add-Type -AssemblyName System.Windows.Forms
            Write-Output "$UserSamAccountName already has $MailBoxPrimarySMTPAddress Calendar Reviewer permissions" 
        ...
        If ($Status.AccessRights -eq 'Owner') {
            Write-Output "$UserSamAccountName already has $MailBoxPrimarySMTPAddress Calendar Owner permissions"
            Add-Type -AssemblyName System.Windows.Forms 
        ...
        Else {
            ...
            #####################################################################
            Add-Type -AssemblyName System.Windows.Forms 
            Write-Output "$UserSamAccountName added to $MailBoxPrimarySMTPAddress Calendar as Reviewer"
...
Function CheckCalPermAccessPermissionForm {
    $WinTitle =  "Calendar - Check current Calendar Permissions v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $CheckCalPermForm.Text = $wintitle
    ...
    if ($dialogResult -eq 'OK') {
        ...
        Write-Output "Listing $MailboxName calendar"
...
Function RemoveCalendarPermAccessPermissionForm {
    $WinTitle =  "Calendar - Remove Calendar Access Permissions for a User v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $RemoveCalendarPermForm.Text = $WinTitle
    ...
    if ($dialogResult -eq 'OK') {
        ...
        Elseif ($RemoveCalendarPermSelectedMailBoxNametextLabel6.Text -eq '') {
            Write-Output "No calender selected"
        ...
        If (($Status.AccessRights -ne 'Owner') -and ($Status.AccessRights -ne 'Reviewer')) {
            Write-Output "user $UserSamAccountName doesn't have access to $MailBoxPrimarySMTPAddress Calendar"
            Add-Type -AssemblyName System.Windows.Forms 
        ...
        Else {
            ...
            Write-output "user $UserSamAccountName removed from $MailBoxPrimarySMTPAddress Calendar"
            #####################################################################
            ##################  Message complete message  #######################
            #####################################################################
            Add-Type -AssemblyName System.Windows.Forms 
...
Function DistributionListAddUserForm {
    $WinTitle =  "Mailbox - Distribution List - Add User to a List v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $DistListAddUserForm.Text = $wintitle
    ...
    if ($dialogResult -eq 'OK') {
        ...
        Elseif ($DistListAddUserSelectedMailBoxNametextLabel6.Text -eq '') {
            Write-output "No Distributuion list selected"
        ...
        If ($DistributionGroupMembers -notcontains $UserSamAccountName) {
            ...
            Add-Type -AssemblyName System.Windows.Forms 
            Write-output "User $UserSamAccountName has been added $DistributionGroupMembers"
        ...
        Else {
            { Add-Type -AssemblyName System.Windows.Forms 
                Write-output "User $UserSamAccountName is already a member of $DistributionGroupMembers"
...
Function DistributionListRemoveUsererForm {
    $WinTitle =  "Mailbox - Distribution List - Remove User from a List v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $DistListRemoveUserForm.Text = $wintitle
    ...
    if ($dialogResult -eq 'OK') {
        ...
        if ($DistListRemoveUserSelectedUserNametextLabel4.Text -eq '') {
            Write-Output "no Username selected"
        ...
        Elseif ($DistListRemoveUserSelectedMailBoxNametextLabel6.Text -eq '') {
            Write-Output "no Distribution List selected"
        ...
        $MailboxSamAccountname = Get-ADGroup -SearchBase 'OU=Legacy,OU=Groups,OU=COURTS,DC=scotcourts,DC=local' -Filter "Name -eq '$($DistListRemoveUserSelectedMailBoxNametextLabel6.Text)'" | Select-Object SamAccountName | Select-Object -ExpandProperty SamAccountName 
        #####################################################################
        #########        Check if User already is a member      #############
        #####################################################################
        ...
        If ($Status1 -eq 0) {
            Add-Type -AssemblyName System.Windows.Forms
            Write-Output "$UserSamAccountName is not a member of group $MailboxSamAccountname"
        ...
        Else {
            ...
            Write-Output "$UserSamAccountName removed from group $MailboxSamAccountname"
            #####################################################################
            ##################  Message complete message  #######################
            #####################################################################
            Add-Type -AssemblyName System.Windows.Forms 
...
Function DistListListUsersForm {
    $WinTitle =  "Mailbox - Distribution List - List current members of a List v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $DistListListForm.Text = $WinTitle
    ...
    if ($dialogResult -eq 'OK') {
        ...
        Write-Output "$DistListName list selected"
...
Function DistListAddNew {
    $WinTitle =  "Add New Distribution List v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $DistListAddNewForm.Text = $WinTitle
    ...
    if ($dialogResult -eq 'OK') {
        ...
        if ($DistListAddNewSelectedUserNametextLabel4.Text -eq '') {
            Write-Output "New Distribution list - no Name address entered"
        ...
        Elseif ($DistListAddNewSelectedMailBoxNametextLabel6.Text -eq '') {
            Write-Output "New Distribution list - no email address entered"
        ...
        $ListEmailAddress = Get-ADObject -Filter "mail -eq '$EmailName@scotcourts.gov.uk'" | Measure-Object count 
        If ($Null -ne $ListEmailAddress) {
            Write-Output "New Distribution list - $EmailName is in use"
            Add-Type -AssemblyName System.Windows.Forms 
        ...
        Else {
            ...
            #####################################################################
            Add-Type -AssemblyName System.Windows.Forms
            Write-Output "New Distribution List - $DisplayName - has been added with email address $EmailName" 
            ...
            ### Build Form ###
            $objForm = New-Object System.Windows.Forms.Form
            $objForm.Text = 'Add New Distribution List'
            $objForm.Size = New-Object System.Drawing.Size(450, 270)
            $objForm.StartPosition = 'CenterScreen'
            $objForm.Controlbox = $false
            #### Add Label ###
...
Function SharedMailboxManagementForm {
    $WinTitle =  "Shared Mailbox - Management v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    # Set the details of the form.
    ...
    $SharedMailboxForm.Text = $WinTitle
    ...
    $CancelButton.Text = 'Cancel back to MainForm'
    $CancelButton.add_Click( {
            Write-Output "Return to mainform"
    ...
    if ($dialogResult -eq 'OK') {
        ...
        if ($SharedMailBoxButton1.Checked) { 
            Write-Output "Add Mailbox Full access Selected"
            AddMBFullAccessPermissionForm 
        }
        elseif ($SharedMailBoxButton2.Checked) { 
            Write-Output "Add Send on behalf Selected"
            AddMBSendBehalfOfAccessPermissionForm 
        }
        elseif ($SharedMailBoxButton3.Checked) { 
            Write-Output "Check Mailbox access Selected"
            CheckMBFullAccessPermissionForm 
        }
        elseif ($SharedMailBoxButton4.Checked) { 
            Write-Output "Check Send on behalf Selected"
            CheckMBSendOnBehalfPermissionForm 
        }
        elseif ($SharedMailBoxButton5.Checked) { 
            Write-Output "Remove Mailbox Full access Selected"
            RemoveMBFullAccessPermissionForm 
        }
        elseif ($SharedMailBoxButton6.Checked) { 
            Write-Output "Remove Send on behalf Selected"
            RemoveMBSendOnBehalfPermissionForm 
        }
        elseif ($SharedMailBoxButton7.Checked) { 
            Write-Output "Access mailbox Selected"
            SharedLogOnForm 
        }
        elseif ($SharedMailBoxButton8.Checked) { 
            Write-Output "Add new Shared Mailbox Selected"
            AddNewSharedMBForm 
        }
        elseif ($SharedMailBoxButton9.Checked) { 
            Write-Output "Add New Tribunals Shared Mailbox Selected"
            AddNewTribunalSharedMBForm 
        }
        elseif ($SharedMailBoxButton10.Checked) { 
            Write-Output "Add send copy Selected"
            CopySharedMBForm 
        }
    }
}
...
Function OutOfficeManagementForm {
    $WinTitle =  "User Mailbox - Out Of Office management v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    # Set the details of the form.
    ...
    $OutOfficeManagementForm.Text = $WinTitle
    ...
    if ($Result -eq 'OK') {
        # Check the current state of each radio button and respond.
        if ($OutOfficeButton1.Checked) { 
            Write-Output "OOO check Selected"
            OutOfficeCheckForm 
        }
        elseif ($OutOfficeButton2.Checked) { 
            Write-Output "Turn ON OOO Selected"
            OutOfficeTurnOnForm 
        }
        elseif ($OutOfficeButton3.Checked) { 
            Write-Output "Turn OFF OOO Selected"
            OutOfficeTurnOffForm 
        }
        elseif ($OutOfficeButton4.Checked) { 
            Write-Output "Add OOO Selected"
            OutOfficeAddTurnOnForm 
        }
        elseif ($OutOfficeButton5.Checked) { 
            Write-Output "See OOO Message Selected"
            OutOfficeCheckMessageForm 
        }
    }
}
#########################################################################################################
...
Function SharedCalendarManagementForm {
    ...
    if ($Result -eq 'OK') {
        # Check the current state of each radio button and respond.
        if ($SharedMailBoxCalendarButton1.Checked) { 
            Write-Output "Add Owner to Calendar Selected"
            AddCalOwnerPermissionForm 
        }
        elseif ($SharedMailBoxCalendarButton2.Checked) { 
            Write-Output "Add reviwer to Calendar Selected"
            AddCalReviewerPermissionForm 
        }
        elseif ($SharedMailBoxCalendarButton3.Checked) { 
            Write-Output "Check Calendar Permissions Selected"
            CheckCalPermAccessPermissionForm 
        }
        elseif ($SharedMailBoxCalendarButton4.Checked = $true) { 
            Write-Output "Remove User from Calendar Selected"
            RemoveCalendarPermAccessPermissionForm 
        }
    }
}
#########################################################################################################
...
Function DistributionListManagementForm {
    $WinTitle =  "Distribution List - Management v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    # Set the details of the form.
    ...
    $DistributionListManagementForm.Text = $WinTitle
    ...
    if ($Result -eq 'OK') {
        # Check the current state of each radio button and respond.
        if ($DistributionListButton1.Checked) { 
            Write-Output "Add User to Distribution list Selected"
            DistributionListAddUserForm 
        }
        elseif ($DistributionListButton2.Checked) { 
            Write-Output "Remove User to Distribution list Selected"
            DistributionListRemoveUsererForm 
        }
        elseif ($DistributionListButton3.Checked) { 
            Write-Output "View Users in Distribution list Selected"
            DistListListUsersForm 
        }
        elseif ($DistributionListButton4.Checked) { 
            Write-Output "Add new Distribution list Selected"
            DistListAddNew 
        }
    }
}
...
Function DisabledUserManagementForm {
    $WinTitle =  "Mailbox - Set Disabled Users mailbox only accept from helpdesk & Hide from Address List v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    $DisabledUserManagementForm.Text = $WinTitle
    ...
    if ($dialogResult -eq 'OK') {
        ...
        Else {
            ...
            If ($Null -eq $MailboxCheck.EmailAddress) {
                Write-Output "Error - $HideAcceptSelectedUserNametextLabel4.Text Does not have a mailbox"
            ...
            Write-output "The mailbox $MailBoxPrimarySMTPAddress has been disabled"
            #####################################################################
            ##################  Message complete message  #######################
            #####################################################################
...
Function MobileResetform {
    $WinTitle =  "Mobile device reset form v$Version"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ### Set the details of the form. ###
    ...
    if ($dialogResult -eq 'OK') {
        ...
        Else {
            ...
            #####################################################################
            $MailboxCheck = Get-ADUser -Filter { DisplayName -eq $HideAcceptSelectedUserNametextLabel4.Text } -Properties * | Select-Object EmailAddress
            $Mailboxactive = Get-ADUser -Filter { DisplayName -eq $HideAcceptSelectedUserNametextLabel4.Text } -Properties * | ForEach-Object {$_.lastLogon}
            If ($Null -eq $MailboxCheck.EmailAddress) {
                Write-Output "Error - $HideAcceptSelectedUserNametextLabel4.Text does not have a mailbox"
            ...
            elseif ($Mailboxactive -eq "0") {
                Write-Output "Error - $HideAcceptSelectedUserNametextLabel4.Text does not have an active mailbox"
                [System.Windows.Forms.MessageBox]::Show("The selected User does not have an active mailbox The options cannot be set.", 'Mobile device reset', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
                $MobileResetform.Close()
                $MobileResetform.Dispose()
                Return MobileResetform
            }
            ...
            Write-Output "Mailbox $MailBoxPrimarySMTPAddress has been enabled for Mobile devices & previous devices removed."
            #####################################################################
            ##################  Message complete message  #######################
            #####################################################################
...
Function MainForm {
    ...
    $Ex2013ManForm.controls.add($pictureBox)
    ...
    $Ex2013ManGroupBox.Location = '40,100'
