...
#               V4.00   13/06/2022  BS  O365 changes
#
$Date = Get-Date -Format "dd-MM-yyyy"
Start-Transcript -Path "\\scotcourts.local\data\CDiScripts\Scripts\Logs\Email\$Date.txt" -append
$Version = '5.00'
...
if ($UserName -notlike "*_a") {
    ...
    function EmailForm {
        ...
        $CancelButton.add_Click( {
                Write-Host "Email selection closed"
        ...
        if ($Result -eq 'OK') {
            if ($BoxRadioButton1.Checked) {
                ...
                Write-Host "$UserSamAccountName@ScotCourts.gov.uk selected as primary"
                ######################################################################
                ...
                Set-ADUser -Identity $UserSamAccountName -clear "extensionattribute2"
                Set-ADUser -Identity $UserSamAccountName -add @{"extensionattribute2" = "USR-PERS" }
                ...
                $NewEmail4 = "$UserSamAccountName@ScotCourtsTribunals.pnn.gov.uk"
                $NewEmail5 = "$UserSamAccountName@scotcourtsgovuk.mail.onmicrosoft.com"
                Write-Host "$UserSamAccountName@ScotCourtsTribunals.gov.uk selected as primary"
                ######################################################################
                $newProxy = "smtp:" + $NewEmail2
                $NewPrimary = "SMTP:" + $NewEmail1
                $newProxy1 = "smtp:" + $NewEmail3
                ...
                Set-ADUser -identity $UserSamAccountName -add @{proxyAddresses = ($newProxy3) }
                Set-ADUser -Identity $UserSamAccountName -clear "extensionattribute2"
                Set-ADUser -Identity $UserSamAccountName -add @{"extensionattribute2" = "USR-PERT" }
                Add-Type -AssemblyName System.Windows.Forms
