# Delete User Accounts for Leavers v2.1
# Author        Brian Stark
# Date          02/09/2020
# Version       2.1
# Purpose       To remove Leavers AD user accounts & Users folders on SAUFS01.
# Useage        helpdesk staff run a shorcut to Removing User Accounts for Leavers.exe
#
# Changes       02/09/2020 BS - 2.1 Update & clean up of script & fix issues.
...
Start-Transcript -Path "\\scotcourts.local\data\IT\Enterprise Team\UserManagement\Logging\Delete\$Date.txt" -append
$message = "
...
if ($StartMessage -eq 'Cancel') {
    ...
    Function DeleteMultipleUserAccount {
        ...
        if ($dialogResult -eq 'OK') {
            Write-Output " Accepted DeleteLeavers form and starting to Remove User AD Account" 
            DeleteUserADAccount
        }
    }
    ...
    Function DeleteUserADAccount {     
        ...
        try {
            ...
            Write-Output "error on $UserToDelete"
    ...
    Function DeleteUserFolders {  
        ...
        ForEach ($folder in $folders) {
            Write-output "Users $folder marked for deletion"
        ...
        Write-Output "The Leavers older than 1 month have been deleted." 
    ...
    if ($null -eq $LeaversOlderThan1MonthList) {
        Write-output "no Accounts due for deletion"
