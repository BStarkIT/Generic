...
Start-Transcript -Path "\\scotcourts.local\data\IT\Enterprise Team\UserManagement\Logging\ICMSDev\$Date.txt" -append
    ...
    if ($dialogResult -eq 'OK') {
        ...
        if ($BoxRadioButton1.Checked) {
            ...
            ForEach ($group in $groups) {
                ...
                Write-Output "$UserSamAccountName Removed from $group" 
            }
