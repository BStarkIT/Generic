...
Start-Transcript -Path "\\scotcourts.local\data\IT\Enterprise Team\UserManagement\Logging\ACL\$Date.txt" -append
#######################################################################################################
...
Function ListACLManagementForm {
    ...
    if ($dialogResult -eq 'OK') {
        ...
        if ($ACLListListSelectedNametextLabel6.Text -eq '') {
            Write-Output "empty list selected"
        ...
        Write-Output "Showing ACL $ACLListName"
...
Function AddtoACLManagementForm {
    ...
    if ($dialogResult -eq 'OK') {
        ...
        ######                Add user        ###############
        #####################################################################
        Write-output "Adding User $AddUser, $UserSamAccountName to ACL $ACLListName"
...
Function RemovefromACLManagementForm {
    ...
    if ($dialogResult -eq 'OK') {
        ...
        ######                Add user        ###############
        #####################################################################
        Write-output "Removing User $RemoveUser, $UserSamAccountName from ACL $ACLListName"
...
Function MainForm {
    ...
    $CancelButton.add_Click( {
            Write-Output "ACL Management closed"
            $ACLManForm.Close()
            $ACLManForm.Dispose()
