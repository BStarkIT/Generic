$Users = Get-ADUser -filter * -searchbase 'ou=soe users 2.6,ou=scts users,ou=user accounts,ou=scts,DC=scotcourts,DC=local' -Properties EmailAddress | Select-Object EmailAddress | Select-Object -ExpandProperty EmailAddress
