...
# Tested date   
# Version       5.00
# Purpose       To control ACL groups.
# Useage        helpdesk staff run a shorcut to Users - ACL.
#
# Changes
#               13/07/2022  BS  5.0 Powershell 7 & path changes
#               V4.00   13/06/2022  BS  O365     
#               V3.00 28/02/2022 BS - Transcript & Single OU settings due to Windows Update requirements.
#               V2.00 25/08/2020 BS - Bring in line with other management scripts
#               V1.01 07/05/2020 BS - Add remove & list fully working
#               V1.00 20/04/2020 BS - Basic outline & basic functions
#               
#
#
# Start of script:
$Date = Get-Date -Format "dd-MM-yyyy"
Start-Transcript -Path "\\scotcourts.local\data\CDiScripts\Scripts\Logs\ACL\$Date.txt" -append
#######################################################################################################
###                 Set icon for all forms and subforms                                             ###
#######################################################################################################
$Version = '5.00'
$WinTitle = "HelpDesk control script v$version."
$Icon = '\\scotcourts.local\data\CDiScripts\Scripts\Resources\Icons\acl.ico'
#######################################################################################################
###                 Get ACL's from AD                                                               ###
#######################################################################################################
...
if ($UserName -notlike "*_a") {
    ...
    Function ListACLManagementForm {
        ...
        if ($dialogResult -eq 'OK') {
            ...
            if ($ACLListListSelectedNametextLabel6.Text -eq '') {
                Write-Host "empty list selected"
            ...
            Write-Host "Showing ACL $ACLListName"
    ...
    Function AddtoACLManagementForm {
        ...
        if ($dialogResult -eq 'OK') {
            ...
            ######                Add user        ###############
            #####################################################################
            Write-Host "Adding User $AddUser, $UserSamAccountName to ACL $ACLListName"
    ...
    Function RemovefromACLManagementForm {
        ...
        if ($dialogResult -eq 'OK') {
            ...
            ######                Add user        ###############
            #####################################################################
            Write-Host "Removing User $RemoveUser, $UserSamAccountName from ACL $ACLListName"
            Remove-ADGroupMember -Identity $ACLListName -Members $UserSamAccountName -Confirm:$false
            #####################################################################
            ##################  Message complete message  #######################
            #####################################################################
            Add-Type -AssemblyName System.Windows.Forms 
    ...
    Function MainForm {
        ...
        $ACLManForm.width = 500
        $ACLManForm.height = 400
        ...
        $ACLManForm.controls.add($pictureBox)
        # Create a group that will contain your radio buttons
        $ACLManGroupBox = New-Object System.Windows.Forms.GroupBox
        $ACLManGroupBox.Location = '40,120'
        $ACLManGroupBox.size = '400,160'
        ...
        $ACLManGroupBoxRadioButton1.size = '300,40'
        ...
        $ACLManGroupBoxRadioButton2.size = '300,40'
        ...
        $ACLManGroupBoxRadioButton3.size = '300,40'
        $ACLManGroupBoxRadioButton3.Checked = $false
        $ACLManGroupBoxRadioButton3.Text = 'Remove Staff from an ACL.'
        # Add an OK button
        $OKButton = New-Object System.Windows.Forms.Button
        $OKButton.Location = '250,300'
        $OKButton.Size = '100,40' 
        $OKButton.Text = 'OK'
        $OKButton.DialogResult = [System.Windows.Forms.DialogResult]::OK
        #Add a cancel button
        $CancelButton = New-Object System.Windows.Forms.Button
        $CancelButton.Location = '350,300'
        $CancelButton.Size = '100,40'
        $CancelButton.Text = 'Exit'
        $CancelButton.add_Click( {
                Write-Host "ACL Management closed"
                $ACLManForm.Close()
                $ACLManForm.Dispose()
