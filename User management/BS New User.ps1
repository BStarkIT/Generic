...
# Useage        helpdesk staff run a shorcut to New User.exe
#               
# Revisions     V1.11   09/06/2021  BS  Added catch for space in names
...
Start-Transcript -Path "\\scotcourts.local\data\IT\Enterprise Team\UserManagement\Logging\New\$Date.txt" -append
...
Function SCTSForm {
    ...
    $OfficeList = Import-Csv "\\saufs01\it\Enterprise Team\UserManagement\Lists\2022\Lists\Office.csv"
    $SecurityGroupsList = Import-Csv "\\saufs01\it\Enterprise Team\UserManagement\Lists\2022\Lists\SecurityGroups.csv"
    $DescriptionList = Import-Csv "\\saufs01\it\Enterprise Team\UserManagement\Lists\2022\Lists\description.csv"
...
Function Tribsform {
    ...
    $OfficeList = Import-Csv "\\saufs01\it\Enterprise Team\UserManagement\Lists\2022\Lists\OfficeTribs.csv"
    $SecurityGroupsList = Import-Csv "\\saufs01\it\Enterprise Team\UserManagement\Lists\2022\Lists\SecurityGroupsTribs.csv"
    $DescriptionList = Import-Csv "\\saufs01\it\Enterprise Team\UserManagement\Lists\2022\Lists\DescriptionTribs.csv"
    $Distributionlists = Get-DistributionGroup | Select-Object Name | Select-Object -ExpandProperty Name
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
...
Function Judform {
    ...
    $OfficeList = Import-Csv "\\saufs01\it\Enterprise Team\UserManagement\Lists\2022\Lists\JudicialUsers\officeJud.csv"
    $SecurityGroupsList = Import-Csv "\\saufs01\it\Enterprise Team\UserManagement\Lists\2022\Lists\SecurityGroups.csv"
    $DescriptionList = Import-Csv "\\saufs01\it\Enterprise Team\UserManagement\Lists\2022\Lists\JudicialUsers\descriptionJud.csv"
    $Distributionlists = Import-Csv "\\scotcourts.local\data\it\Enterprise Team\UserManagement\Lists\2022\Lists\JudicialUsers\distributionJud.csv"
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
...
function SBuildForm {
    ...
    New-AdUser -Name "$DisplayName" -SamAccountName "$SAM" -GivenName "$FirstName" -Surname "$Lastname" -DisplayName "$DisplayName" -UserPrincipalName "$SAM@scotcourts.gov.uk" -Office $Office -Description $Description  -Path $OU -Enabled $True -ChangePasswordAtLogon $false  -AccountPassword (ConvertTo-SecureString "Helpdesk123" -AsPlainText -force) -passThru
    ...
    Set-CASMailbox -Identity $SAM -PopEnabled $False -OWAEnabled $False -ImapEnabled $False -ActiveSyncEnabled $False
...
Function MainForm {
    ...
    if ($Result -eq 'OK') {
        if ($textBox1.Text -eq '') {
            Write-output "No Firstname entered"
        ...
        elseif ($textBox2.Text -eq '') {
            Write-output "No Surname entered"
            [System.Windows.Forms.MessageBox]::Show("You need to enter a user name!  Trying to enter blank fields is never a good idea.", $WinTitle, [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
            $ManForm.Close()
            $ManForm.Dispose()
            break
        ...
        elseif ($FirstName -match " ") {
            Write-output "space found in first name"
        ...
        elseif ($LastName -match " ") {
            Write-output "space found in last name"
            [System.Windows.Forms.MessageBox]::Show("Space cannot be in a name", $WinTitle, [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
            $ManForm.Close()
            $ManForm.Dispose()
            break
        ...
        Else {
            if ($Result -eq 'OK') {
                if ($BoxRadioButton1.Checked) {
                    ...
                    $mail = "$tentativeSAM@ScotCourts.gov.uk"
                    $SAM = $tentativeSAM
                    Write-output "$DisplayName Requested, Sam $SAM & Email $mail"
                    if ($ContBox.Checked) {
                        SCTSContForm
                    }
                    else {
                ...
                elseif ($BoxRadioButton3.Checked = $True) {
                    if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
                        ...
                        elseif (($Title -eq "Sheriff") -or ($Title -eq "Sheriffs") -or ($Title -eq "Sheriffp")) {
                            ...
                            Write-output "No title selected"
