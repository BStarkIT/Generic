...
#                Creates new user in the Tribunals\NewUsers OU in AD with profile & user folder on saufs01.
...
#                A log is kept in \\saufs01\it\Enterprise Team\UserManagement\Add-New-User-With-Mailbox\NewTribsUserLog.txt
# Usage          Helpdesk staff run a shorcut to Add-New-User-With-Mailbox-Tribunals.exe in \\saufs01\it\Enterprise Team\UserManagement\Add-New-User-With-Mailbox.
# Changes        v1.02 JM changed send email to helpdesk to use $env:UserName instead of helpdesk.
...
#
Start-Transcript -Path "\\saufs01\it\Enterprise Team\UserManagement\Add-New-User-With-Mailbox\Logs\NewTribsUserLog.txt" -append
########################################################################
...
$StartMessage = [System.Windows.Forms.MessageBox]::Show("This script creates a New Tribunal User Account with a mailbox in Exchange 2013.`n`nThe New Account will be created in the Tribunals\NewUsers OU in AD & the account will be disabled.`n`nBefore use the New User Account needs to be moved from the Tribunals\NewUsers OU to the correct Tribunal User OU in AD & enabled.`n`nPlease click OK to continue or Cancel to exit", "Add New Tribunal User Account with Exchange 2013 mailbox.", [System.Windows.Forms.MessageBoxButtons]::OKCancel, [System.Windows.Forms.MessageBoxIcon]::Warning)
...
Function AddNewUser{
    ...
    $NewUserForm1.Text = "Add New Tribunal User Account with Exchange 2013 mailbox v1.02"
    ...
    $DistributionList = Import-csv "\\saufs01\it\Enterprise Team\UserManagement\Lists\DistributionTribs.csv"
    ...
    #############        Create AD account         ###########################
    ##########################################################################
    New-AdUser  -Name "$DisplayName" -SamAccountName $LogonName -Path "OU=NewUsers,OU=Tribunals,DC=scotcourts,DC=local" –AccountPassword ($Password | ConvertTo-SecureString -AsPlainText –Force)
    ##########################################################################
    #
    ##########################################################################
    ...
    ##########################################################################
    Add-Type -AssemblyName System.Windows.Forms 
    $StartMessage = [System.Windows.Forms.MessageBox]::Show("The User account and mailbox have been created in the Tribunals NewUsers OU.`n`nNote1:  The user account needs to be moved from the Tribunals NewUsers OU to the correct Tribunal user OU in AD.:`n`nNote2:  The user account needs to be enabled before use.", "New Tribunal User Account", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
    ##########################################################################
    ###########      Send email to helpdesk             ######################
    ##########################################################################
    Send-MailMessage -To helpdesk@scotcourts.gov.uk -from $env:UserName@scotcourts.gov.uk -Subject "HDupdate: New Tribunal User Account $LogOnName added. The new User needs moved out of the Tribunal New User OU in AD." -Body "A new Tribunal user account has been added:`n`nUserName:   $DisplayName`n`nLogOnName:   $LogOnName`n`nLocation:   $Description`n`nDistribution Lists:  All Users Tribunals  $DistributionGroup`n`nSecurity Groups:  acl_All STS Users_readwrite, acl_All_Tribunals_Users, $SecurityGroup " -SmtpServer mail.scotcourts.local
    Remove-Variable DisplayName
    AddNewUser}
    }}
    AddNewUser
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUuVoR7cXtNo/WmXuG6upIVMkl
# UQ6ggg19MIIGYDCCBUigAwIBAgITbwAAACIK+5RtwBxYlgAAAAAAIjANBgkqhkiG
...
# DjAMBgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBQK1yoFXy5a7Yg3mAp/+CcA
# K0tsXjANBgkqhkiG9w0BAQEFAASCAQA+014pdFhW3FGd/O532/OpBL7NPye+iAvr
# ELvaTGT1bil9rbj9vKcMkVnpmwebkuy5PjAImg+0rNqJPTA/nV9NZ/E4qrAXpypF
# dHMb3TosMkH7T7ueI/JQ5NwlBBvhc87zJt+mdu7icv4PDwCgID+kZAw4BRZ5Ac2P
# xTVRaHeLv248/poV42X8RZ50PTXy2SnPf2ly/7zW45Pw2AzhYwgW6Vevlyyv5eeC
# g8Y4Dc6Lpe91egOQjr1bO7s/O0+QU7SW5lOriGZ/rQ0k5kKPH2QftMzBIa34OypN
# AND/3+lBlWkNKNkV8hjcbZbKZOJtD0PHEX3QrSpUvTqwjdkXP8nh
