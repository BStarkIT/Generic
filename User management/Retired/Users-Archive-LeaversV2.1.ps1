# Archive User Accounts for Leavers v2.3
# Author        Brian Stark
# Date          02/09/2020
# Version       2.3
...
#               Fork of the disable 
#
# Changes       V2.3 15/09/2020 BS fork for Archive
#               V2.2 02/09/2020 BS Update & correct script
...
AD account - Labels account as ARCHIVE.
...
SAUFS01 - Labels user Profile and P drive folders as Archived.
...
if ($StartMessage -eq 'Cancel') {
    ...
    Function PopUpForm {
        ...
        $PopForm.Text = "Archive User Account for Leavers."
    ...
    Function LeaverProcess {
        ...
        $LeaverProcessForm.Text = "Archive User Account for Leavers v$version."
        ...
        if ($dialogResult -eq 'OK') {
            ...
            $poplabel = "Renaming Users Profile folder and P drive folders`n`non SAUFS01`n`nwith Archive."
            PopupForm
            try {
                if (test-path \\scotcourts.local\data\profiles\"$UserSamAccountName.v2") {
                    Get-Item \\scotcourts.local\data\profiles\"$UserSamAccountName.v2" | Rename-Item -NewName {$_.Name -replace "$", " X Archive X"}
            ...
            try {
                if (test-path \\scotcourts.local\Home\P\$UserSamAccountName) {
                    Get-Item \\scotcourts.local\home\P\$UserSamAccountName | Rename-Item -NewName {$_.Name -replace "$", " X Archive X"}
            ...
            $poplabel = "Editing users AD account:`n`nLabelling as Archived.`n`nClearing Profile Path and P drive path."
            PopupForm
            try {
                Get-ADUser $UserSamAccountName -Properties Description | ForEach-Object {Set-ADUser $_ -Description "$($_.Description) X Archived X"}
                ...
                Write-Verbose "Labelling users AD account As Archived and clearing Profile and P drive paths" -Verbose
            }
            catch {
