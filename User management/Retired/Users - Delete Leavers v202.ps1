# Delete User Accounts for Leavers v2.02
# Author        John Mckay
# Date          06/12/2018
# Version       2.02
...
#
# Changes       06/12/2018 JM - 1. user accounts with blackberries won't delete with "remove-aduser" changed to "remove-adobject" & added DeleteUserADAccount function.
...
SAUFS01: Users P drive folder on \\scotcourts.local\data\users will be deleted."
#
# Start of script:
#
##  Show Start Message:   ###
[System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms') | Out-Null 
$StartMessage = [System.Windows.Forms.MessageBox]::Show("$message", 'Delete User Accounts for Leavers.', [System.Windows.Forms.MessageBoxButtons]::OKCancel, [System.Windows.Forms.MessageBoxIcon]::Information)
if ($StartMessage -eq 'Cancel') {Exit} 
    ...
    Function DeleteMultipleUserAccount {
        ...
        $DeletingUsersForm.Text = 'Delete Leavers User Accounts v2.02.'
    ...
    Function DeleteUserADAccount {     
        ...
        try {
            ForEach ($UserToDelete in $LeaversOlderThan1MonthList) {
                ...
                #Remove-ADUser -Identity $UserToDelete.SamAccountName -Confirm:$false
    ...
    Function DeleteUserFolders {  
        ...
        $DeletedUserList = ""
        $path = "\\scotcourts.local\data\users"
        ...
        ForEach ($folder in $folders) {
            ...
            If ($FolderDate -lt $DateToDel) {
                ## delete user folder on scotcourts.local\data\users   ###
                try {
                    if (test-path \\scotcourts.local\data\users\$($folder.PSChildname)) {
                        ...
                        $poplabel = "Deleting the User`n`n$($folder.PSChildname)`n`nP drive folder on \\scotcourts.local\data\users."
                        PopupForm
                    }
                }       
                catch {
    ...
    Function DeleteProfileFolders {  
        ...
        ForEach ($Profilefolder in $folders) {
            ...
            If ($FolderDate -lt $DateToDel) {
                ...
                try {
                    ...
                    [System.Windows.Forms.MessageBox]::Show("Something has gone WRONG removing the User`n`n$($Profilefolder.PSChildname)`n`nProfile folder !!!.`n`nPlease contact the Systems Integration Team with the details.", 'Delete User Accounts for Leavers.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                    $DeletingUsersForm.Close()
                    break  
                }
            }
        ...
        [System.Windows.Forms.MessageBox]::Show("The Delete Leavers process is complete.`n`nA list of deleted user accounts will be emailed.", 'Delete leavers.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
        break
    }    
    ...
    $LeaversList = Get-ADUser -Filter * -SearchBase "OU=Z-Disabled_Leavers,OU=UserAccounts,OU=courts,DC=scotcourts,DC=local" -Properties Name, description, DistinguishedName |
    ...
    if ($LeaversOlderThan1MonthList -eq $null) {
        [System.Windows.Forms.MessageBox]::Show("There are currently no Leavers to delete.`n`nThe Delete Leavers process is complete.", 'Delete Leavers.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
        break
    }
