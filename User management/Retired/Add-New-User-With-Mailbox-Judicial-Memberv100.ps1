...
$StartMessage = [System.Windows.Forms.MessageBox]::Show("This script creates a New Judicial User Account with a mailbox in Exchange 2013.`n`nThe New Judicial Account will be created in the NewUsers OU in AD & the account will be disabled.`n`nBefore use the New Judicial User Account needs to be moved from the NewUsers OU to the correct User OU in AD & enabled.`n`nPlease click OK to continue or Cancel to exit", "Add New Judicial User Account with Exchange 2013 mailbox.", [System.Windows.Forms.MessageBoxButtons]::OKCancel, [System.Windows.Forms.MessageBoxIcon]::Warning)
...
####################################################################
    $JudFirstName = Import-csv "\\saufs01\it\Enterprise Team\UserManagement\Lists\JudicialUsers\FirstNameJud.csv"
    ...
    $distributionList = Import-csv "\\saufs01\it\Enterprise Team\UserManagement\Lists\JudicialUsers\distributionJud.csv"
    ...
    $NewUserForm1.Text = "Add New Judicial User Account with Exchange 2013 mailbox v1.00"
    ...
    #####################################################################
    if ($NewUsertextBox2.text -eq "") {[System.Windows.Forms.MessageBox]::Show("You need to type in details !!!!!`n`nTrying to enter blank fields is never a good idea.", "Add New Judicial User", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    ...
    #####################################################################
    ElseIf (($Firstname -eq "sheriff") -or ($Firstname -eq "sheriffs") -or ($Firstname -eq "sheriffp")  -and ($Initial -eq "")){[System.Windows.Forms.MessageBox]::Show("You need to type in an INITIAL !!!!!`n`nSheriffs need to have an initial.", "Add New Judicial User", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    Remove-Variable FirstName, LastName, Initial
    $NewUserForm1.Close()
    $NewUserForm1.Dispose()
    AddNewJudicialUser}
    ...
    #####################################################################
    If (($FirstName -eq "Lord") -or ($FirstName -eq "Lady") -and ($Initial -ne "")){[System.Windows.Forms.MessageBox]::Show("You have selected a Judge with an INITIAL !!!!!`n`nJudges don't have initials added to their accounts.", "Add New Judicial User", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    Remove-Variable FirstName, LastName, Initial
    $NewUserForm1.Close()
    $NewUserForm1.Dispose()
    AddNewJudicialUser}
    ...
    Else {$DisplayName = $NewUsertextLabel6.Text
    ...
    $objLabel.Text = "A New Judicial User Account is being created in AD`nwith the details you entered.`n`nThe New Account will be created in the NewUser OU.`n`nPlease Wait. .............."
    ...
    ##########################################################################
    Add-Type -AssemblyName System.Windows.Forms 
    [System.Windows.Forms.MessageBox]::Show("The Judicial User account and mailbox have been created in the NewUsers OU.`n`nNote1:  The Judicial user account needs to be moved from the NewUsers OU to the correct user OU in AD.:`n`nNote2:  The Judicial user account needs to be enabled before use.", "New Judicial User Account", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
    ##########################################################################
