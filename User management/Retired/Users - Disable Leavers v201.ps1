# Disable User Accounts for Leavers v2.01
# Author        John Mckay
# Date          11/09/2018
# Version       2.01
...
$StartMessage = [System.Windows.Forms.MessageBox]::Show("This script performs the following on a User account." + "`n" + "$message", 'Disable User Account for Leavers.', [System.Windows.Forms.MessageBoxButtons]::OKCancel, [System.Windows.Forms.MessageBoxIcon]::Information)
if ($StartMessage -eq 'Cancel') {Exit} 
    ...
    Function LeaverProcess {
        ...
        $LeaverProcessForm.Text = 'Disable User Account for Leavers v2.01.'
        ...
        if ($dialogResult -eq 'OK') {
            ...
            if ($LeaverSelectedMailBoxNametextLabel6.Text -eq '') {
                [System.Windows.Forms.MessageBox]::Show("You need to select a User !!!!!`n`nTrying to enter blank fields is never a good idea.`n`nProcessing cannot continue.", 'Disable User Accounts for Leavers.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                $LeaverProcessForm.Close()
                $LeaverProcessForm.Dispose()
                break
            }
            ...
            If ($MailboxCheck.EmailAddress -eq $null) {
                [System.Windows.Forms.MessageBox]::Show("The selected User does not have a mailbox`n`nProcessing cannot continue.", 'Disable User Accounts for Leavers.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                $LeaverProcessForm.Close()
                $LeaverProcessForm.Dispose()
            ...
            PopupForm
            $Usermembership = get-aduser -identity $UserSamAccountName -property MemberOf | Foreach-Object {$_.MemberOf | Get-AdGroup | Select-Object Name, SamaccountName | Select-object -ExpandProperty SamAccountName}
            $pdrive = test-path \\scotcourts.local\data\users\$UserSamAccountName
            try {
                    If (($Usermembership -ne $null) -and ($pdrive -eq $true)) {                    
                    $Usermembership | out-file \\scotcourts.local\data\users\$UserSamAccountName\UserMembershipBackup.csv
                    Write-Verbose "Backed up Users Security group and Distribution List membership to users P drive." -Verbose
                }
            }
            catch {
                [System.Windows.Forms.MessageBox]::Show("Something has gone WRONG backing up the users group membership !!!.`n`nPlease contact the Systems Integration Team with the details.", 'Disable User Accounts for Leavers.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                $LeaverProcessForm.Close()
                Return LeaverProcess  
            }
            #
            ...
            try {
                ...
                [System.Windows.Forms.MessageBox]::Show("Something has gone WRONG with renaming the users profile folder !!!.`n`nPlease contact the Systems Integration Team with the details.", 'Processing User Accounts for Leavers.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                $LeaverProcessForm.Close()
                Return LeaverProcess
            }
            try {
                if (test-path \\scotcourts.local\data\users\$UserSamAccountName) {
                    Get-Item \\scotcourts.local\data\users\$UserSamAccountName | Rename-Item -NewName {$_.Name -replace "$", " xx DELETE after DATE - $Datedel"}
                    Write-Verbose "Renamed users P drive folder on SAUFS01" -Verbose
                }
            }
            catch {
                [System.Windows.Forms.MessageBox]::Show("Something has gone WRONG with renaming the users P drive !!!.`n`nPlease contact the Systems Integration Team with the details.", 'Processing User Accounts for Leavers.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                $LeaverProcessForm.Close()
                Return LeaverProcess
            }
            #
            ...
            try {
                ...
                [System.Windows.Forms.MessageBox]::Show("Something has gone WRONG setting the users mailbox !!!.`n`nPlease contact the Systems Integration Team with the details.", 'Processing User Accounts for Leavers.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                $LeaverProcessForm.Close()
                Return LeaverProcess  
            }
            #
            ...
            try {
                ...
                [System.Windows.Forms.MessageBox]::Show("Something has gone WRONG disabling the users AD account !!!.`n`nPlease contact the Systems Integration Team with the details.", 'Processing User Accounts for Leavers.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                $LeaverProcessForm.Close()
                Return LeaverProcess 
            }
            #
            ...
            try {
                ...
                [System.Windows.Forms.MessageBox]::Show("Something has gone WRONG labelling the users AD account to be deleted in 1 month !!!.`n`nPlease contact the Systems Integration Team with the details.", 'Disable User Accounts for Leavers.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                $LeaverProcessForm.Close()
                Return LeaverProcess
            }
            #
            ## move ad account to Z-Disabled Leavers ou   ###
            $poplabel = "Moving user account to the `n`nCourts/UserAccounts/Z-Disabled_Leavers OU."
            PopupForm
            try {
                Get-ADUser $UserSamAccountName | Move-ADObject -targetpath 'OU=Z-Disabled_Leavers,OU=UserAccounts,OU=courts,DC=scotcourts,DC=local'
                Write-Verbose "moving Users AD account to Z-Disabled_Leavers OU" -Verbose
            }
            catch {
                [System.Windows.Forms.MessageBox]::Show("Something has gone WRONG moving the users account to the Z-Disabled_Leavers OU !!!.`n`nPlease contact the Systems Integration Team with the details.", 'Disable User Accounts for Leavers.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                $LeaverProcessForm.Close()
                Return LeaverProcess 
            }
            #
            ...
            try {
                ...
                [System.Windows.Forms.MessageBox]::Show("Something has gone WRONG removing the users shared mailbox permissions !!!.`n`nPlease contact the Systems Integration Team with the details.", 'Disable User Accounts for Leavers.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                $LeaverProcessForm.Close()
                Return LeaverProcess
            }
            #
            ...
            try {
                ...
                [System.Windows.Forms.MessageBox]::Show("Something has gone WRONG removing the users from the groups !!!.`n`nPlease contact the Systems Integration Team with the details.", 'Disable User Accounts for Leavers.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                $LeaverProcessForm.Close()
                Return LeaverProcess
            }
            [System.Windows.Forms.MessageBox]::Show("Disable User Accounts for Leavers for $($LeaverMBNameComboBox1.SelectedItem.ToString()) - Completed.", 'Disable User Accounts for Leavers.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
            $LeaverProcessForm.Close()
            Return LeaverProcess
