    ...
    Function DeleteUserADAccount {     
        ...
        try {
            ForEach ($UserToDelete in $LeaversOlderThan1MonthList) {
                ...
                Get-ADUser -Filter "Displayname -eq '$($UserToDelete.Name)'" | Select-Object -ExpandProperty 'DistinguishedName' | Remove-ADObject -Recursive
        ...
        ForEach ($Profilefolder in $folders) {
            ...
            If ($FolderDate -lt $DateToDel) {
                ...
                try {
                    ...
                    Write-Verbose "The Profile folder on SAUFS01 for user $($Profilefolder.PSChildname) has not been deleted." -Verbose
                }
            }
        }
        # Details of deleted user folders to include in email to helpdesk   #
