# Exchange 2013 Management v1.44
# Author        John Mckay
# Date          18/07/2018
# Version       1.44
...
#
# Changes       v1.44 03/09/2018 JM - can't add send on behalf permissions for more than 1 user on new shared mailboxes. Can add more than 1 on existing mailboxes. Edited match in line 414 to If ($Status -match "$UserDisplayName.*")
...
## Layout of file:
#
# Main Form which contains 6 forms.
#
# 1.Shared Mailbox – User Access management
# 2.User Mailbox - Out of Office Management
# 3.Shared Calendar – management
# 4.Distribution list – management
# 5.Disabled user Account – mailbox management
#
# Subforms contain powershell commands.
#
# 1.1 Add- Full Access permissions for a User, 1.2 Add- Send On Behalf Of permissions for a User, 1.3 Check - Mailbox current Full Access permissions, 1.4 Check - Mailbox current Send On Behalf Of permissions,
#          1.5 Remove- Full Access permissions for a User, 1.6 Remove - Send On Behalf Of permissions for a User, 1.7 Shared Mailbox LogOn 1.8 Add New Courts Shared Mailbox 1.9 Add New Tribunals shared mailbox
#
# 2.1 Out of Office - check current status, 2.2 OOO - Turn On, 2.3 OOO - Turn Off, 2.4 OOO - Add message and turn On, 2.5 OOO - Check current message
#
# 3.1 Add - owner permissions, 3.2 Add - reviewer permissions, 3.3 Check - current calendar permissions, 3.4 Remove - remove calendar permissions for a user
#
# 4.1 Distribution List - Add User to a List, 4.2 Distribution List - Remove User from a List, 4.3 Distribution List - List current members of a List, 4.4 Distribution List - Add New Distribution List 
#
# 5.1 Mailbox - Set Disabled User's mailbox only accept from helpdesk & Hide from Address List
#
#
...
#########################################################################################################
$Sharedmailboxes = Get-Mailbox -OrganizationalUnit 'ou=shared mailboxes,ou=resource accounts,ou=useraccounts,ou=courts,dc=scotcourts,dc=local' | Select-Object DisplayName | Select-Object -ExpandProperty DisplayName 
$Distributionlists = Get-AdGroup -Filter * -SearchBase 'ou=distribution lists,ou=groups,ou=courts,dc=scotcourts,dc=local' | Select-Object Name | Select-Object -ExpandProperty Name  
#########################################################################################################
...
$UserNameList = $Users1 + $users2 +$users3 +$users4 + $users5 + $users6 + $users7 + $users8 + $users9 + $users10 + $users11 + $users12
#########################################################################################################
#
#########################################################################################################
...
Function AddMBFullAccessPermissionForm{
    ...
    #####################################################################
    if ($AddMBFullSelectedUserNametextLabel4.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Username !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Mailbox - Add Full Access Permissions for a User.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $AddMBFullForm.Close()
    $AddMBFullForm.Dispose()
    break}
    Elseif ($AddMBFullSelectedMailBoxNametextLabel6.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Shared mailbox !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Mailbox - Add Full Access Permissions for a User.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $AddMBFullForm.Close()
    $AddMBFullForm.Dispose()
    break}
    #####################################################################
    ...
    #####################################################################
    Add-Type -AssemblyName System.Windows.Forms 
    [System.Windows.Forms.MessageBox]::Show("The user ( $($AddMBFullUserNameComboBox1.SelectedItem.ToString()) ) has had Full Access Permissions added to the ( $($AddMBFullMBNameComboBox2.SelectedItem.ToString()) ) mailbox.`n`nThe user needs to close and re-open Outlook and`n`nthe mailbox will appear in a few minutes.", 'Mailbox - Add Full Access Permissions for a User', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    #####################################################################
    #############   Send email to helpdesk    ###########################
    #####################################################################
    Send-MailMessage -To helpdesk@scotcourts.gov.uk -from $env:UserName@scotcourts.gov.uk -Subject "HDupdate: The User $($AddMBFullUserNameComboBox1.SelectedItem.ToString()) has had Full Access permissions added to the $($AddMBFullMBNameComboBox2.SelectedItem.ToString()) mailbox" -Body 'The user needs to close and re-open Outlook and`n`nthe mailbox will appear in a few minutes.' -SmtpServer mail.scotcourts.local
    $AddMBFullForm.Close()
    $AddMBFullForm.Dispose()
    Return SharedMailboxManagementForm}
}}
...
Function AddMBSendBehalfOfAccessPermissionForm{
    ...
    if ($dialogResult -eq 'OK') {
        ...
        if ($AddMBSendBehalfOfSelectedUserNametextLabel4.Text -eq '') {
            [System.Windows.Forms.MessageBox]::Show("You need to select a Username !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Mailbox - Add Send On Behalf Of Access Permissions for a User.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
            $AddMBSendBehalfOfForm.Close()
            $AddMBSendBehalfOfForm.Dispose()
            break
        }
        Elseif ($AddMBSendBehalfOfSelectedMailBoxNametextLabel6.Text -eq '') {
            [System.Windows.Forms.MessageBox]::Show("You need to select a Shared mailbox !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Mailbox - Add Send On Behalf Of Permissions for a User.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
            $AddMBSendBehalfOfForm.Close()
            $AddMBSendBehalfOfForm.Dispose()
            break
        }
...
Function CheckMBFullAccessPermissionForm{
    ...
    #####################################################################
    if ($CheckMBFullSelectedMailBoxNametextLabel6.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Mailbox !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Mailbox -Check Full Access Permissions on Mailbox.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $CheckMBFullForm.Close()
    $CheckMBFullForm.Dispose()
    break}
    #####################################################################
...
Function CheckMBSendOnBehalfPermissionForm{
    ...
    ########           Don't accept null mailbox         ################ 
    #####################################################################
    if ($CheckMBSendOnBehalfSelectedMailBoxNametextLabel6.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Mailbox !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Check Send On Behalf Of permissions on Mailbox', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $CheckMBSendOnBehalfForm.Close()
    $CheckMBSendOnBehalfForm.Dispose()
    break}
    #####################################################################
...
Function RemoveMBFullAccessPermissionForm{
    ...
    ########   Don't accept null username or mailbox     ################ 
    #####################################################################
    if ($RemoveMBFullSelectedUserNametextLabel4.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Username !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Mailbox - Remove Full Access Permissions for a User.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $RemoveMBFullForm.Close()
    $RemoveMBFullForm.Dispose()
    break}
    Elseif ($RemoveMBFullSelectedMailBoxNametextLabel6.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Shared mailbox !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Mailbox - Remove Full Access Permissions for a User.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $RemoveMBFullForm.Close()
    $RemoveMBFullForm.Dispose()
    break}
    #####################################################################
...
Function RemoveMBSendOnBehalfPermissionForm{
    ...
    if ($dialogResult -eq 'OK') {
        ...
        if ($RemoveMBSendOnBehalfSelectedUserNametextLabel4.Text -eq '') {
            [System.Windows.Forms.MessageBox]::Show("You need to select a Username !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Mailbox - Remove Send On Behalf Of permissions for a User.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
            $RemoveMBSendOnBehalfForm.Close()
            $RemoveMBSendOnBehalfForm.Dispose()
            break
        }
        Elseif ($RemoveMBSendOnBehalfSelectedMailBoxNametextLabel6.Text -eq '') {
            [System.Windows.Forms.MessageBox]::Show("You need to select a Shared mailbox !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Mailbox - Remove Send On Behalf Of permissions for a User.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
            $RemoveMBSendOnBehalfForm.Close()
            $RemoveMBSendOnBehalfForm.Dispose()
            break
        }
...
Function SharedLogOnForm {
    ...
    $StartMessage = [System.Windows.Forms.MessageBox]::Show("This script manages Shared Mailboxes.`n`nNote 1: When prompted for a username & password enter:`n`nscs\YourLogOnName & password`n`nNote 2:When prompted for an Outlook profile add a new profile named:`n`nSharedMailbox`n`nNote 3:When adding new profile the email address needs changed to Shared Mailbox Name`n`nPlease click OK to continue or Cancel to exit", 'Shared Mailbox - Manage Shared Mailboxes.', [System.Windows.Forms.MessageBoxButtons]::OKCancel, [System.Windows.Forms.MessageBoxIcon]::Warning)
    if ($StartMessage -eq 'Cancel') { exit } 
    ### Check if Outlook running  ###
    if ((get-process "outlook" -ea SilentlyContinue) -ne $Null) {
        Add-Type -AssemblyName System.Windows.Forms 
        ...
        if ($dialogResult -eq 'OK') {
            ...
            if ($SharedLogOnSelectedMailBoxNametextLabel6.Text -eq '') {
                [System.Windows.Forms.MessageBox]::Show("You need to select a Mailbox !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Shared Mailbox - Manage Shared Mailboxes.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
                $SharedLogOnForm.Close()
                $SharedLogOnForm.Dispose()
                Return SharedMailboxManagementForm
            }
            Else {
                ...
                $UserSamName = $UserName -replace "^.*\\", "" -replace "}", ""
                $SID = (get-aduser -Identity $UserSamName).SID.Value
                ### CHECK - continue if only 1 mailbox & sid  ###
                if (($UserName | Measure-Object).count -ne 1) {SharedLogOn}
                ...
                If ((Test-Path -Path "Registry::\HKEY_USERS\$SID\Software\Microsoft\Windows NT\CurrentVersion\Windows Messaging Subsystem\Profiles\SharedMailbox") -eq '$true') {
                    Remove-Item -Path "Registry::\HKEY_USERS\$SID\Software\Microsoft\Windows NT\CurrentVersion\Windows Messaging Subsystem\Profiles\SharedMailbox" -Recurse
                }
                $arguments = '-profiles'
                Start-Process -FilePath 'C:\program files (x86)\microsoft office\office14\OUTLOOK.EXE' -ArgumentList $arguments -Credential (Get-Credential) -Wait
                Remove-Variable arguments
                ### If outlook closed without creating profile exit  ###
                If ((Test-Path -Path "Registry::\HKEY_USERS\$SID\Software\Microsoft\Windows NT\CurrentVersion\Windows Messaging Subsystem\Profiles\SharedMailbox") -ne '$true') {
                    [System.Windows.Forms.MessageBox]::Show("Outlook has been closed without adding a SharedMailbox profile`n`nYou need to add an outlook profile to manage a shared mailbox.", "Shared Mailbox - Manage Shared Mailboxes.", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
...
Function AddNewSharedMBForm{
    ...
    #########################################################################
    Add-Type -AssemblyName System.Windows.Forms 
    $StartMessage = [System.Windows.Forms.MessageBox]::Show("This script creates a New Shared Mailbox.`n`nThe new mailbox will be created in the Courts\Users\Shared Mailboxes OU in AD.`n`nPlease Note 1: There are 4 fields & all the fields are mandatory.`n`nPlease note 2: The Display Name will appear in the global address list and should start with a capital letter.`n`nPlease Note 3: The email address entered doesnt need @scotcourts.gov.uk added and should have no spaces or non-alpha characters.`n`nPlease click OK to continue or Cancel to exit", 'Add New Shared Mailbox.', [System.Windows.Forms.MessageBoxButtons]::OKCancel, [System.Windows.Forms.MessageBoxIcon]::Warning)
    if ($StartMessage -eq 'Cancel') { exit } 
    else {
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ...
    ########   Don't accept null username or mailbox     ################ 
    #####################################################################
    if ($SharedMBNewtextBox1.text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to type in details !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Add New Shared Mailbox', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $SharedMBNewForm.Close()
    $SharedMBNewForm.Dispose()
    break}
    #####################################################################
    ...
    #####################################################################
    Add-Type -AssemblyName System.Windows.Forms 
    [System.Windows.Forms.MessageBox]::Show("New Shared Mailbox - $DisplayName - has been added.`n`nThe users who need:`n`nFull Access and`n`nSend on Behalf Of permissions`n`nnow need to be added to the new mailbox.", 'Add New Shared Mailbox', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    #####################################################################
    ...
    $objLabel.Text = 'The Shared Mailbox List is being reloaded from AD`n`n with the new mailbox you have just added. `n`n`n`nPlease Wait. ..............'
    ...
    #############   Send email to helpdesk    ###########################
    #####################################################################
    Send-MailMessage -To helpdesk@scotcourts.gov.uk -from $env:UserName@scotcourts.gov.uk -Subject "HDupdate: New Shared Mailbox - $DisplayName - has been added" -Body "New Shared mailbox - $DisplayName - has been added`n`nThe users who need:`n`nFull Access and`n`nSend on Behalf Of permissions`n`nnow need to be added to the new mailbox." -SmtpServer mail.scotcourts.local
    $SharedMBNewForm.Close()
    $SharedMBNewForm.Dispose()
    Return SharedMailboxManagementForm}
    }}
...
Function AddNewTribunalSharedMBForm{
    ...
    #########################################################################
    Add-Type -AssemblyName System.Windows.Forms 
    $StartMessage = [System.Windows.Forms.MessageBox]::Show("This script creates a New Tribunal Shared mailbox.`n`nThe new mailbox will be created in the Courts\Users\Shared mailboxes OU in AD.`n`nPlease Note 1: There are 4 fields & all the fields are mandatory.`n`nPlease note 2: The Display Name will appear in the global address list and should start with a capital letter.`n`nPlease Note 3: The email address entered doesnt need @scotcourts.gov.uk added and should have no spaces or non-alpha characters.`n`nPlease click OK to continue or Cancel to exit", 'Add New Tribunal Shared mailbox.', [System.Windows.Forms.MessageBoxButtons]::OKCancel, [System.Windows.Forms.MessageBoxIcon]::Warning)
    if ($StartMessage -eq 'Cancel') { exit } 
    else {
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms')
    [void] [System.Reflection.Assembly]::LoadWithPartialName('System.Drawing') 
    ...
    ########   Don't accept null username or mailbox     ################ 
    #####################################################################
    if ($TribSharedMBNewtextBox1.text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to type in details !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Add New Tribunal Shared mailbox', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $TribSharedMBNewForm.Close()
    $TribSharedMBNewForm.Dispose()
    break}
    #####################################################################
    ...
    #####################################################################
    Add-Type -AssemblyName System.Windows.Forms 
    [System.Windows.Forms.MessageBox]::Show("New Tribunal Shared mailbox - $DisplayName - has been added.`n`nThe users who need:`n`nFull Access and`n`nSend on Behalf Of permissions`n`nnow need to be added to the new mailbox.", 'Add New Tribunal Shared mailbox', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    #####################################################################
    #############   Reload Distribution lists       ####################
    #####################################################################
    ###  Create form to pause for 7 sec  ###
    ...
    $objLabel.Location = New-Object System.Drawing.Size(80,50) 
    $objLabel.Size = New-Object System.Drawing.Size(300,120)
    $objLabel.Text = 'The Tribunal Shared mailbox List is being reloaded from AD`n`n with the new mailbox you have just added. `n`n`n`nPlease Wait. ..............'
    $objForm.Controls.Add($objLabel)
    ### Show the form ###
    $objForm.Show()| Out-Null
    ### wait 7 seconds ###
    ...
    #############   Send email to helpdesk    ###########################
    #####################################################################
    Send-MailMessage -To helpdesk@scotcourts.gov.uk -from $env:UserName@scotcourts.gov.uk -Subject "HDupdate: New Tribunal Shared mailbox - $DisplayName - has been added" -Body "New Tribunal Shared mailbox - $DisplayName - has been added`n`nThe users who need:`n`nFull Access and`n`nSend on Behalf Of permissions`n`nnow need to be added to the new mailbox." -SmtpServer mail.scotcourts.local
    $TribSharedMBNewForm.Close()
    $TribSharedMBNewForm.Dispose()
    Return SharedMailboxManagementForm}
    }}
...
Function OutOfficeCheckForm{
    ...
    ########           Don't accept null mailbox         ################ 
    #####################################################################
    if ($OutOfficeCheckSelectedMailBoxNametextLabel6.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a UserName !!!!!`n`nTrying to enter blank fields is never a good idea.", 'User Mailbox - Out Of Office - Check current status.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $OutOfficeCheckForm.Close()
    $OutOfficeCheckForm.Dispose()
    break}
    #####################################################################
    ...
    Add-Type -AssemblyName System.Windows.Forms 
    [System.Windows.Forms.MessageBox]::Show("The Out of Office for user $($OutOfficeCheckMBNameComboBox2.SelectedItem.ToString()) is currently:`n`n`n                   DISABLED - turned off.", 'User Mailbox - Out Of Office - Check current status.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    }
    ElseIf ($Status.AutoReplyState -eq 'Enabled'){
    Add-Type -AssemblyName System.Windows.Forms 
    [System.Windows.Forms.MessageBox]::Show("The Out of Office for use $($OutOfficeCheckMBNameComboBox2.SelectedItem.ToString()) is currently:`n`n`n                    ENABLED - turned on.", 'User Mailbox - Out Of Office - Check current status.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    }
    $OutOfficeCheckForm.Close()
    $OutOfficeCheckForm.Dispose()
    Return OutOfficeManagementForm}
...
Function OutOfficeTurnOnForm{
    ...
    ########           Don't accept null mailbox         ################ 
    #####################################################################
    if ($OutOfficeTurnOnSelectedMailBoxNametextLabel6.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a UserName !!!!!`n`nTrying to enter blank fields is never a good idea.", 'User Mailbox - Out Of Office - Turn On.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $OutOfficeTurnOnForm.Close()
    $OutOfficeTurnOnForm.Dispose()
    break}
    #####################################################################
    ...
    Add-Type -AssemblyName System.Windows.Forms 
    [System.Windows.Forms.MessageBox]::Show("The Out of Office for user $($OutOfficeTurnOnMBNameComboBox2.SelectedItem.ToString())`n`n`n      is already Enabled and turned on.", 'User Mailbox - Out Of Office - Turn On.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $OutOfficeTurnOnForm.Close()
    $OutOfficeTurnOnForm.Dispose()
    Return OutOfficeManagementForm}
    Else {
    ...
    Add-Type -AssemblyName System.Windows.Forms 
    [System.Windows.Forms.MessageBox]::Show("The Out of Office for user $($OutOfficeTurnOnMBNameComboBox2.SelectedItem.ToString()) has been`n`n`n     ENABLED - turned on.", 'User Mailbox - Out Of Office - Turn On.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $OutOfficeTurnOnForm.Close()
    $OutOfficeTurnOnForm.Dispose()
    Return OutOfficeManagementForm}
    }
...
Function OutOfficeTurnOffForm{
    ...
    ########           Don't accept null mailbox         ################ 
    #####################################################################
    if ($OutOfficeTurnOffSelectedMailBoxNametextLabel6.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a UserName !!!!!`n`nTrying to enter blank fields is never a good idea.", 'User Mailbox - Out Of Office - Turn Off.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $OutOfficeTurnOffForm.Close()
    $OutOfficeTurnOffForm.Dispose()
    break}
    #####################################################################
    ...
    If ($Status.AutoReplyState -eq 'Disabled'){
    Add-Type -AssemblyName System.Windows.Forms 
    [System.Windows.Forms.MessageBox]::Show("The Out of Office for user $($OutOfficeTurnOffMBNameComboBox2.SelectedItem.ToString())`n`n`n      is already Disabled and turned off.", 'User Mailbox - Out Of Office - Turn Off', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $OutOfficeTurnOffForm.Close()
    $OutOfficeTurnOffForm.Dispose()
    Return OutOfficeManagementForm}
    Else {
    ...
    Add-Type -AssemblyName System.Windows.Forms 
    [System.Windows.Forms.MessageBox]::Show("The Out of Office for user $($OutOfficeTurnOffMBNameComboBox2.SelectedItem.ToString()) has been`n`n`n     DISABLED - turned off.", 'User Mailbox - Out Of Office - Turn Off.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $OutOfficeTurnOffForm.Close()
    $OutOfficeTurnOffForm.Dispose()
    Return OutOfficeManagementForm}
    }
...
Function OutOfficeAddTurnOnForm{
    ...
    #####################################################################
    if ($OutOfficeAddUserSelectedUserNametextLabel4.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Username !!!!!`n`nTrying to enter blank fields is never a good idea.", 'User Mailbox - Out Of Office - Add message and turn On.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $OutOfficeAddUserForm.Close()
    $OutOfficeAddUserForm.Dispose()
    break}
    Elseif ($OutOfficeMessagetextBox2.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to enter a message !!!!!`n`nTrying to enter blank fields is never a good idea.", 'User Mailbox - Out Of Office - Add message and turn On.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $OutOfficeAddUserForm.Close()
    $OutOfficeAddUserForm.Dispose()
    break}
    #####################################################################
    ...
    #####################################################################
    Add-Type -AssemblyName System.Windows.Forms 
    [System.Windows.Forms.MessageBox]::Show("The message:`n`n`n$($OutOfficeMessagetextBox2.Text)`n`n`nhas been added to the Out of Office for $($OutOfficeAddUserUserNameComboBox1.SelectedItem.ToString())`n`n`nand has been ENABLED - turned on.", "User Mailbox - Out Of Office - Add message and turn On.", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    #####################################################################
    #############   Send email to helpdesk    ###########################
    #####################################################################
...
Function OutOfficeCheckMessageForm{
    ...
    ########           Don't accept null mailbox         ################ 
    #####################################################################
    if ($OutOfficeCheckMessageSelectedMailBoxNametextLabel6.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a UserName !!!!!`n`nTrying to enter blank fields is never a good idea.", 'User Mailbox - Out Of Office - Check Message.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $OutOfficeCheckMessageForm.Close()
    $OutOfficeCheckMessageForm.Dispose()
    break}
    #####################################################################
...
Function AddCalOwnerPermissionForm{
    ...
    #####################################################################
        ...
        [System.Windows.Forms.MessageBox]::Show("You need to select a Username !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Calendar - Add Owner Permissions for a User.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
        $AddCalOwnerForm.Close()
        $AddCalOwnerForm.Dispose()
        break
        }
        Elseif ($AddCalOwnerSelectedMailBoxNametextLabel6.Text -eq '') {
        [System.Windows.Forms.MessageBox]::Show("You need to select a Calendar !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Calendar - Add Owner Permissions for a User.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
        $AddCalOwnerForm.Close()
        $AddCalOwnerForm.Dispose()
        break
        }
    ...
    #####################################################################
        ...
        [System.Windows.Forms.MessageBox]::Show("The user ( $($AddCalOwnerUserNameComboBox1.SelectedItem.ToString()) ) already has REVIEWER Permissions to the ( $($AddCalOwnerMBNameComboBox2.SelectedItem.ToString()) ) calendar.`n`nRemove the REVIEWER permissions before adding Owner permissions", 'Calendar - Add Owner Permissions for a User.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
        $AddCalOwnerForm.Close()
        $AddCalOwnerForm.Dispose()
        Return SharedCalendarManagementForm
        }
        ...
        #####################################################################
        Add-Type -AssemblyName System.Windows.Forms 
        [System.Windows.Forms.MessageBox]::Show("The user ( $($AddCalOwnerUserNameComboBox1.SelectedItem.ToString()) ) has had Owner Permissions added to the ( $($AddCalOwnerMBNameComboBox2.SelectedItem.ToString()) ) calendar.`n`nThe user needs to close and re-open Outlook`n`nto pick up the permissions.", 'Calendar - Add Owner Permissions for a User.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
        #####################################################################
        #############   Send email to helpdesk    ###########################
        #####################################################################
...
Function AddCalReviewerPermissionForm{
    ...
    ########   Don't accept null username or mailbox     ################ 
    #####################################################################
        if ($AddCalReviewerSelectedUserNametextLabel4.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Username !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Calendar - Add Reviewer Permissions for a User.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
        $AddCalReviewerForm.Close()
        $AddCalReviewerForm.Dispose()
            break
        }
        Elseif ($AddCalReviewerSelectedMailBoxNametextLabel6.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Calendar !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Calendar - Add Reviewer Permissions for a User.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
        $AddCalReviewerForm.Close()
        $AddCalReviewerForm.Dispose()
            break
        }
        ...
        If ($Status.AccessRights -eq 'Owner') {
            Add-Type -AssemblyName System.Windows.Forms 
            [System.Windows.Forms.MessageBox]::Show("The user ( $($AddCalReviewerUserNameComboBox1.SelectedItem.ToString()) ) already has OWNER Permissions to the ( $($AddCalReviewerMBNameComboBox2.SelectedItem.ToString()) ) calendar.`n`nRemove the OWNER permissions before adding Reviewer permissions", "Calendar - Add Reviewer Permissions for a User", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
            $AddCalReviewerForm.Close()
            $AddCalReviewerForm.Dispose()
            Return SharedCalendarManagementForm
        }
        Else {
            ...
            #####################################################################
            Add-Type -AssemblyName System.Windows.Forms 
            [System.Windows.Forms.MessageBox]::Show("The user ( $($AddCalReviewerUserNameComboBox1.SelectedItem.ToString()) ) has had Reviewer Permissions added to the ( $($AddCalReviewerMBNameComboBox2.SelectedItem.ToString()) ) calendar.`n`nThe user needs to close and re-open Outlook`n`nto pick up the permissions.", 'Calendar - Add Reviewer Permissions for a User', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
            #####################################################################
            #############   Send email to helpdesk    ###########################
            #####################################################################
...
Function CheckCalPermAccessPermissionForm{
    ...
    ########           Don't accept null mailbox         ################ 
    #####################################################################
    if ($CheckCalPermSelectedMailBoxNametextLabel6.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Calendar !!!!!n`nTrying to enter blank fields is never a good idea.", 'Calendar - Check current Calendar Permissions.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $CheckCalPermForm.Close()
    $CheckCalPermForm.Dispose()
    break}
    #####################################################################
...
Function RemoveCalendarPermAccessPermissionForm{
    ...
    ########   Don't accept null username or mailbox     ################ 
    #####################################################################
    if ($RemoveCalendarPermSelectedUserNametextLabel4.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Username !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Calendar - Remove Calendar Access Permissions for a User.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $RemoveCalendarPermForm.Close()
    $RemoveCalendarPermForm.Dispose()
    break}
    Elseif ($RemoveCalendarPermSelectedMailBoxNametextLabel6.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Calendar !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Calendar - Remove Calendar Access Permissions for a User.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $RemoveCalendarPermForm.Close()
    $RemoveCalendarPermForm.Dispose()
    break}
    #####################################################################
...
Function DistributionListAddUserForm{
    ...
    ########   Don't accept null username or mailbox     ################ 
    #####################################################################
    if ($DistListAddUserSelectedUserNametextLabel4.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Username !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Mailbox - Distribution List - Add User to a List.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $DistListAddUserForm.Close()
    $DistListAddUserForm.Dispose()
    break}
    Elseif ($DistListAddUserSelectedMailBoxNametextLabel6.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Distribution List !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Mailbox - Distribution List - Add User to a List.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $DistListAddUserForm.Close()
    $DistListAddUserForm.Dispose()
    break}
    #####################################################################
    ...
    $MailboxSamAccountname = Get-ADGroup -SearchBase 'OU=Distribution Lists,OU=Groups,OU=COURTS,DC=scotcourts,DC=local' -Filter "Name -eq '$($DistListAddUserSelectedMailBoxNametextLabel6.Text)'" | Select-Object SamAccountName | Select-Object -ExpandProperty SamAccountName 
    #####################################################################
    ...
    $Status1 = ($Status -match "$($DistListAddUserSelectedUserNametextLabel4.Text)").Count
    If($Status1 -eq 1)
    {Add-Type -AssemblyName System.Windows.Forms 
    ...
    ######                Add user to list                ###############
    #####################################################################
    Add-ADGroupMember -Identity $MailboxSamAccountname -Member $UserSamAccountName
    #####################################################################
    ##################  Message complete message  #######################
    #####################################################################
    Add-Type -AssemblyName System.Windows.Forms 
    ...
    #############   Send email to helpdesk    ###########################
    #####################################################################
    Send-MailMessage -To helpdesk@scotcourts.gov.uk -from $env:UserName@scotcourts.gov.uk -Subject "HDupdate: The User $($DistListAddUserUserNameComboBox1.SelectedItem.ToString()) has been added to Distribution list $($DistListAddUserMBNameComboBox2.SelectedItem.ToString())." -Body 'The user needs to close and re-open Outlook.' -SmtpServer mail.scotcourts.local
    $DistListAddUserForm.Close()
    $DistListAddUserForm.Dispose()
    Return DistributionListManagementForm}
    }}
...
Function DistributionListRemoveUsererForm{
    ...
    ########   Don't accept null username or mailbox     ################ 
    #####################################################################
    if ($DistListRemoveUserSelectedUserNametextLabel4.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Username !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Mailbox - Distribution List - Remove User from a List.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $DistListRemoveUserForm.Close()
    $DistListRemoveUserForm.Dispose()
    break}
    Elseif ($DistListRemoveUserSelectedMailBoxNametextLabel6.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Distribution List !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Mailbox - Distribution List - Remove User from a List.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $DistListRemoveUserForm.Close()
    $DistListRemoveUserForm.Dispose()
    break}
    #####################################################################
...
Function DistListListUsersForm{
    ...
    ########           Don't accept null mailbox         ################ 
    #####################################################################
    if ($DistListListSelectedNametextLabel6.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Distribution list !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Mailbox - Distribution List - List current members of a List', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $DistListListForm.Close()
    $DistListListForm.Dispose()
    break}
    #####################################################################
    ...
    Get-ADGroup -SearchBase 'OU=Distribution Lists,OU=Groups,OU=COURTS,DC=scotcourts,DC=local' -Filter "Name -eq '$($DistListName)'" | Select-Object SamAccountName | Get-ADGroupMember | Select-Object Name | Out-GridView -Title "List of current members of the $DistListName distribution list" -Wait
    $DistListListForm.Close()
    $DistListListForm.Dispose()
    Return DistributionListManagementForm}
    }
...
Function DistListAddNew{
    ...
    ########   Don't accept null username or mailbox     ################ 
    #####################################################################
    if ($DistListAddNewSelectedUserNametextLabel4.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to type a Display Name !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Add New Distribution List', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $DistListAddNewForm.Close()
    $DistListAddNewForm.Dispose()
    break}
    Elseif ($DistListAddNewSelectedMailBoxNametextLabel6.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to type an Email address name !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Add New Distribution List.', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $DistListAddNewForm.Close()
    $DistListAddNewForm.Dispose()
    break}
    #####################################################################
    ...
    If ($ListEmailAddress -ne $Null) {
    Add-Type -AssemblyName System.Windows.Forms 
    [System.Windows.Forms.MessageBox]::Show("The Distribution list - $DisplayName - can not be added because the email address $EmailName@scotcourts.gov.uk is currently in use on another list`n`nPlease use a name/email address thats not currently in use.", 'ERROR - CANT ADD NEW DISTRIBUTION LIST', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    ...
    #####################################################################
    New-DistributionGroup -Name $DisplayName -OrganizationalUnit 'ou=Distribution Lists,ou=Groups,ou=courts,dc=scotcourts,dc=local' -SamAccountName $EmailName -Type Distribution
    #####################################################################
    ##################  Message complete message  #######################
    #####################################################################
    Add-Type -AssemblyName System.Windows.Forms 
    [System.Windows.Forms.MessageBox]::Show("New Distribution List - $DisplayName - has been added`n`nThe members now need to be added to the list.", 'Add New Distribution List', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    #####################################################################
    #############   Reload Distribution lists       ####################
    #####################################################################
    #####################################################################
    ...
    $objLabel.Location = New-Object System.Drawing.Size(80,50) 
    $objLabel.Size = New-Object System.Drawing.Size(300,120)
    $objLabel.Text = 'The Distribution Lists are being reloaded from AD with the`n`new List you have just added. `n`n`n`nPlease Wait. ..............'
    $objForm.Controls.Add($objLabel)
    ### Show the form ###
    $objForm.Show()| Out-Null
    ### wait 7 seconds ###
    ...
    function changeit {  
    $script:Distributionlists = Get-AdGroup -Filter * -SearchBase 'ou=distribution lists,ou=groups,ou=courts,dc=scotcourts,dc=local' | Select-Object Name | Select-Object -ExpandProperty Name  
    }  
    changeit 
    #####################################################################
    #############   Send email to helpdesk    ###########################
    #####################################################################
    Send-MailMessage -To helpdesk@scotcourts.gov.uk -from $env:UserName@scotcourts.gov.uk -Subject "HDupdate: New Distribution List - $DisplayName - has been added" -Body "New Distribution List - $DisplayName - has been added`n`nThe members now need to be added to the list." -SmtpServer mail.scotcourts.local
    $DistListAddNewForm.Close()
    $DistListAddNewForm.Dispose()
    Return DistributionListManagementForm}
    }}
...
Function SharedMailboxManagementForm {
    ...
    $SharedMailBoxBox.Controls.AddRange(@($SharedMailBoxButton1, $SharedMailBoxButton2, $SharedMailBoxButton3, $SharedMailBoxButton4, $SharedMailBoxButton5, $SharedMailBoxButton6, $SharedMailBoxButton7, $SharedMailBoxButton8, $SharedMailBoxButton9))
...
Function DisabledUserManagementForm{
    ...
    ########           Don't accept null mailbox         ################ 
    #####################################################################
    if ($HideAcceptSelectedUserNametextLabel4.Text -eq '') {[System.Windows.Forms.MessageBox]::Show("You need to select a Username !!!!!`n`nTrying to enter blank fields is never a good idea.", 'Mailbox - Set Disabled Users mailbox only accept from helpdesk & Hide from Address List', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $DisabledUserManagementForm.Close()
    $DisabledUserManagementForm.Dispose()
    break}
    Else {
    ...
    #####################################################################
    $MailboxCheck = Get-ADUser -filter {DisplayName -eq $HideAcceptSelectedUserNametextLabel4.Text} -Properties * | Select EmailAddress
    If ($MailboxCheck.EmailAddress -eq $null) {[System.Windows.Forms.MessageBox]::Show("The selected User does not have a mailbox`n`nThe options cannot be set.", 'Mailbox - Set Disabled Users mailbox only accept from helpdesk & Hide from Address List', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    $DisabledUserManagementForm.Close()
    $DisabledUserManagementForm.Dispose()
    ...
    #####################################################################
    #Add-Type -AssemblyName System.Windows.Forms 
    [System.Windows.Forms.MessageBox]::Show("The user ( $($HideAcceptUserNameComboBox1.SelectedItem.ToString()) ) has been removed from the address list`n`nand can now only accept emails from the helpdesk.", 'Mailbox - Set Disabled Users mailbox only accept from helpdesk & Hide from Address List', [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
    #####################################################################
    ############### send email to helpdesk        #######################
    #####################################################################
    $DateToDelete = Get-date -date $(get-date).adddays(30) -format D
    Send-MailMessage -To helpdesk@scotcourts.gov.uk -from $env:UserName@scotcourts.gov.uk -Subject "Delete user account $($HideAcceptUserNameComboBox1.SelectedItem.ToString()) on $DateToDelete" -Body "The mailbox for $($HideAcceptUserNameComboBox1.SelectedItem.ToString()) has been hidden from the Address Lists`n`nIt is now only accepting incoming email from the IT helpdesk`n`nThe user account should be deleted on $DateToDelete" -SmtpServer mail.scotcourts.local
    $DisabledUserManagementForm.Close()
    $DisabledUserManagementForm.Dispose()
    Return MainForm}
    }}
...
Function MainForm{
    ...
    $Ex2013ManForm.Text = 'Exchange 2013 Management Main Form v1.44'
    ...
    if ($Result -eq 'OK'){
        ...
        elseif ($Ex2013ManGroupBoxRadioButton5.Checked = $True){
