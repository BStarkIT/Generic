...
$Sharedmailbox3 = Get-Mailbox -OrganizationalUnit 'OU=Shared Mailboxes,OU=Resource Accounts,OU=SCTS,DC=scotcourts,DC=local' | Select-Object DisplayName | Select-Object -ExpandProperty DisplayName 
$Sharedmailboxes = $Sharedmailbox2 + $Sharedmailbox1 + $Sharedmailbox3
...
#$resource = Get-ADUser –Filter * -SearchBase 'ou=ResourceMailboxes,DC=scotcourts,DC=local' -Properties DisplayName | Select-Object Displayname | Select-Object -ExpandProperty DisplayName
#########################################################################################################
#
#endregion Variables
#region Subforms
...
Function DistListAddNew {
    ...
    if ($dialogResult -eq 'OK') {
        ...
        Else {
            ...
            #####################################################################
            New-DistributionGroup -Name $DisplayName -OrganizationalUnit 'ou=Legacy,ou=Groups,ou=SCTS,dc=scotcourts,dc=local' -SamAccountName $EmailName -Type Distribution
            #####################################################################
            ##################  Message complete message  #######################
            #####################################################################
            Add-Type -AssemblyName System.Windows.Forms 
