...
$file = "\\scotcourts.local\data\IT\Civil Lab\00. Civil Lab Overall Delivery\Entitlement Attestation\ICMS\Completed Audits\reformatted\$Site.xlsx"
...
ForEach ($CurrentUser in (Get-ADGroupMember $group5 | Select-Object -exp samaccountname)) {
    if ($CurrentUser -NotIn $users5) {
        Write-Output "user $CurrentUser Missing, removing from group"
        #Remove-ADGroupMember -Identity $group5 -Members $CurrentUser
    }  
}
...
Get-ADGroupMember $group4 | Select-Object -exp samaccountname
